...
import com.sandu.cloud.pay.config.GatewayPayConfig;
import com.sandu.cloud.pay.dto.PayTradeRefundQueryVo;
import com.sandu.cloud.pay.model.PayTradeRefundRecord;
    ...
    *//**
    ...
    public void asyncQueryAliRefundResult(String payTradeNo,Object refundPayTime){
        ...
        if (isSuccess){
            ...
            //step4.获取通知内部系统所需参数
            Map<String,String> internalNotifyParams = this.getInternalSystemNotifyParams(resultMap, record);

            //step5.通知内部系统(订单,充值等)
            Map<String,String> ret = notifyInternalSystem(record.getNotifyUrl(), internalNotifyParams);

            //step6.记录内部系统通知日志
            saveInternalSystemNotifyLog(record.getId(), JsonUtils.toJson(internalNotifyParams),JsonUtils.toJson(ret));
    ...
    private Map<String, String> getInternalSystemNotifyParams(Map resultMap, PayTradeRefundRecord record) {
        Map<String,String> notifyParams = new HashMap<String, String>();
        Map map = (Map)resultMap.get("alipay_trade_fastpay_refund_query_response");
