...
/*    @Resource
    ...
    public void reduceStock() {
        ...
        try{
            bargainService.modifyVitualCount();
        }catch (Exception e){
    ...
    public void sendActBargainProgressTemplateMsg() {
        ...
        try{
        	List<Bargain> actList = bargainService.listWillExpireList();
			if (actList != null && actList.size() > 0) {
				for (Bargain actEntity : actList) {
					List<BargainRegistration> regList = bargainRegistrationService.list(actEntity.getId());
					if(regList!=null && regList.size()>0) {
						for(BargainRegistration regEntity:regList) {
							//未完成的任务进行通知
							if(regEntity!=null && regEntity.getCompleteStatus()==BargainRegistration.COMPLETE_STATUS_UNFINISH) {
								//构建模板消息数据
