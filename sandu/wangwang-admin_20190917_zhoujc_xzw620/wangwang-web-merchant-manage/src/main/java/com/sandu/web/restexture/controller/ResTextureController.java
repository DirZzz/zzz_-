...
//                .companyId()
     ...
     * 公开/取消公开材质
     * @author: chenm
     * @date: 2019-06-19 17:34
     * @param ids 材质id集合,以逗号分隔
     * @param operationType 操作类型:{0:取消公开,1:公开}
     */
    @ApiOperation("批量公开/取消公开材质")
    @PostMapping("/updateSecrecyFlag")
    public ReturnData batchUpdateSecrecyFlag(String ids,Integer operationType){
        if(StringUtils.isBlank(ids) || operationType == null){
            return ReturnData.builder().success(false).code(PARAM_ERROR).message("参数不能为空");
        }
        if(operationType != 1 && operationType != 0){
            return ReturnData.builder().success(false).code(PARAM_ERROR).message("无效的操作类型");
        }
        List<Integer> textureIdList = Utils.getIntegerListFromStr(ids);
        if(CollectionUtils.isEmpty(textureIdList)){
            return ReturnData.builder().success(false).code(PARAM_ERROR).message("参数不能为空");
        }
        LoginUser user = LoginContext.getLoginUser(LoginUser.class);
        resTextureService.batchUpdateSecrecyFlag(textureIdList,operationType,user.getLoginName());
