...
public class GroupProductController extends BaseController {
    ...
    private ReturnData managePutGroup(@RequestBody @Valid GroupPutUpdate put) {
        ...
        if (put.getGroupIds() != null && !put.getGroupIds().isEmpty()) {
            sycMessageDoSend(SyncMessage.ACTION_UPDATE, put.getGroupIds());
        }
        return builder().code(ResponseEnum.SUCCESS);
    }
    ...
    public ReturnData addGroupProduct(@RequestPart("file") MultipartFile file, @Valid GroupAdd groupAdd, BindingResult result) {
        ...
        sycMessageDoSend(SyncMessage.ACTION_ADD,Collections.singletonList(groupId));
    ...
    public ReturnData deleteProductById(String groupIds) {
        ...
        List<Integer> ids = Arrays.stream(groupIds.split(COMMA)).map(item -> Integer.parseInt(item.trim())).collect(Collectors.toList());
        int count = groupProductBizService.deleteGroupByIds(ids);
        if (count > 0) {
            sycMessageDoSend(SyncMessage.ACTION_DELETE, ids);
            return builder().code(ResponseEnum.SUCCESS);
        }
    ...
    public ReturnData secrecyProductByIds(@RequestBody @Valid GroupSecrecyUpdate update, BindingResult result) {
        ...
        if (flag) {
            sycMessageDoSend(SyncMessage.ACTION_UPDATE, update.getGroupIds());
            return builder().code(ResponseEnum.SUCCESS);
        }
        return builder().code(ResponseEnum.ERROR).message("删除失败...");
    }
    ...
    private void sycMessageDoSend(Integer messageAction, List<Integer> ids) {
        rabbitUtils.sycMessageDoSend(ids, messageAction, "GP-" + System.currentTimeMillis(),
                SyncMessage.GROUP_PRODUCT_MODULE, Constants.PLATFORM_CODE_MERCHANT_MANAGE);
    }
}
