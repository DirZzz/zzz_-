...
public class ProductController extends BaseController {
    ...
    public Object getProductInfoById(@ApiParam(value = "要查看的产品ID", required = true) @PathVariable long productId,
        ...
        if (Objects.equals(vo.getSeriesId(), 0)) {
            vo.setSeriesId(null);
        }
		if (!Objects.equals(vo.getModelingId(), 0)) {
    ...
    @RequiresPermissions({"product:view", "platform:factory", "platform:design"})
    @ApiOperation(value = "产品库公司对应品牌列表", response = ProductListVO.class)
    @GetMapping("/list/library/brandList")
    public ReturnData queryLibraryBrandIdList(@Valid ProductQuery productQuery, BindingResult bindingResult) {
        if (bindingResult.hasErrors()) {
            return processValidError(bindingResult, builder());
        }
        productQuery.setQueryType("library");
    ...
    private ReturnData listProduct(ProductQuery productQuery) {
        ...
        productQueryPO.setIsComplexParquet(productQuery.getIsComplexParquet());
        productQueryPO.setExcludeProductIds(Stream.of(Strings.nullToEmpty(productQuery.getExcludeProductIds()).split(Punctuation.COMMA)).filter(StringUtils::isNoneBlank).map(Integer::new).collect(Collectors.toList()));
    ...
    public ReturnData saveProduct(@Valid @RequestBody ProductAdd add, BindingResult result) {
        ...
        if (checkBedProps(add.getProductSmallType(), add.getProductSKUInfos())) {
            return builder().code(ResponseEnum.ERROR).message("此类产品请输入完整的产品属性！");
        }
        log.info("{}", add);
        ...
        try {
			...
			return data.success(false).message(e.getMessage()).code(ERROR);
		} catch (Exception e) {
    ...
    public ReturnData updateProduct(@Valid @RequestBody ProductLibraryUpdate update, BindingResult validResult) {
        ...
        log.error("更新产品参数=" + new Gson().toJson(update));
    ...
    private ReturnData doUpdate(@Valid @RequestBody ProductUpdatePO update, ReturnData data) {
		...
		boolean b = false;
		try {
			b = productBizService.updateProductBiz(update);
		} catch (IllegalStateException e) {
			return data.success(false).message(e.getMessage()).code(ERROR);
		} catch (Exception e) {
			log.error("更新产品异常：{}", e.getMessage());
		}
...
//        String url = "https://fullsearchsync.ci.sanduspace.com/fullsearch-app/sync/product/syncProductInfoListImmediately";
    ...
    @ApiOperation(value = "修复产品数据", response = ReturnData.class)
    @GetMapping("/processSplitTexturesInfo")
    public ReturnData processSplitTexturesInfo() {
    	List<Product> productList = productBizService.getSplitTexturesInfo();
    	if(productList!=null) {
    		productBizService.processSplitTexturesInfo(productList);
    	}
    	return ReturnData.builder().code(SUCCESS).success(false);
    }


    @ApiOperation(value = "同步产品信息到spu中", response = ReturnData.class)
    @GetMapping("/sync/goods/{productIds}")
    public ReturnData syncProductInfoToGoods(@PathVariable("productIds") String productIds) {
		List<Integer> collect = Stream.of(Strings.nullToEmpty(productIds).split(Punctuation.COMMA)).filter(StringUtils::isNoneBlank).map(Integer::valueOf).collect(Collectors.toList());
        if (collect.isEmpty()) {
			return ReturnData.builder().code(NOT_CONTENT).success(true);
        }
        SysUser user = LoginContext.getLoginUser(SysUser.class);
		if (user == null) {
			return ReturnData.builder().success(false).message("请登录...");
		}
        Integer i = productBizService.syncProductInfoToGoods(collect, user.getId());

		return ReturnData.builder().code(i > 0 ? SUCCESS : NOT_CONTENT).success(true);
    }


    @PostMapping("merge/hard")
    @RequiresPermissions({"product:product_merge"})
	@ApiOperation(value = "合并、取消合并产品", response = ReturnData.class)
    public ReturnData mergeProductToHardProduct(@RequestBody @Valid ProductMerge productMerge) {

        LoginUser loginUser = LoginContext.getLoginUser(LoginUser.class);
        productMerge.setLoginUser(loginUser);
        String message = "success";
        Boolean success = true;
        try {
        	productBizService.mergeProducts(productMerge);
		}catch (Exception e){
        	message = e.getMessage();
        	success = false;
		}

		return ReturnData.builder().message(message).code(SUCCESS).success(success);
    }
