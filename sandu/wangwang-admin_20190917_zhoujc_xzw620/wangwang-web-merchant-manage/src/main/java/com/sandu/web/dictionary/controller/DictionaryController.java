...
import com.sandu.api.dictionary.output.PriceUnitVO;
...
public class DictionaryController {
    ...
    @ApiOperation(value = "获取产品售价单位", response = PriceUnitVO.class)
    ...
    public ReturnData listSaleUnit() {
        ...
        List<DictionaryVO> ret = new LinkedList<>();
        for (Dictionary tmp : productUnitPrices) {
            PriceUnitVO vo = new PriceUnitVO();
            vo.setId(tmp.getValue());
            vo.setName(tmp.getName());
            //用于存储适用于此单位的所有产品小类编码
            vo.setSuitableCodes(tmp.getAtt3Info());
            ret.add(vo);
        }
    ...
    public ReturnData designStyle(HttpServletRequest request) {
        RestTemplate restTemplate = new RestTemplate();
        String url = String.format("https://rendertest.sanduspace.com" + "/app/web/design/preRenderResult/getList.htm?houseId=%s&hasPrice=true", 178315);
        HttpHeaders headers = new HttpHeaders();
        String token = request.getHeader("Authorization");
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("Authorization", token);
        HttpEntity<String> requestEntity = new HttpEntity<String>(null, headers);
        ResponseEntity<ResponseEnvelope> exchange = restTemplate.exchange(url, HttpMethod.GET, requestEntity, ResponseEnvelope.class);

        ResponseEnvelope body = exchange.getBody();
...
//        return ReturnData.builder().code(SUCCESS).success(true).data(Arrays.asList(productType, modelType));
                ...
                styles.stream().map(dic ->
                        {
                            ...
                            tvo.setValueKey(dic.getValuekey());
                            return tvo;
                        }
    ...
    @ApiOperation(value = "获取材质细分类")
    @GetMapping("/texture/classification")
    public ReturnData textureClassification() {
        List<Dictionary> styles = dictionaryService.listByType("classification");
        return mergeDicData(styles);
    }

    @ApiOperation(value = "获取材质色系")
    @GetMapping("/texture/colorSystem")
    public ReturnData textureColorSystem() {
        List<Dictionary> styles = dictionaryService.listByType("colorSystem");
        return mergeDicData(styles);
    }
