...
public class BasesupplydemandController extends BaseController {
    ...
    @ApiOperation(value = "删除供求信息", response = SupplyCategoryListVO.class)
    @DeleteMapping(value = "/del")
    public ReturnData delList(String ids) {
        List<Integer> idList = Stream.of(Strings.nullToEmpty(ids).split(Punctuation.COMMA))
        ...
        int count;
        try {
            count = baseSupplydemandBizService.deleteByList(idList);
        } catch (Exception e) {
            e.printStackTrace();
            return ReturnData.builder().message("删除失败...");
        }
        return ReturnData.builder().message(String.format("删除%s数据成功!!", count));
    }
