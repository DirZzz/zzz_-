...
import static com.sandu.service.decoratecustomer.impl.biz.DecorateCustomerBizServiceImpl.gson;
...
public class ProductBizServiceImpl implements ProductBizService {
	...
	private final BaseProductCountertopsService baseProductCountertopsService;
...
//	private final ExecutorService executorService = Executors.newFixedThreadPool(30);
    ...
    public ProductBizServiceImpl(
			...
			ModelAreaTextureRelService modelAreaTextureRelService,
			BaseProductCountertopsService baseProductCountertopsService) {
		...
		this.baseProductCountertopsService = baseProductCountertopsService;
    }
	...
	private void dealWithCeilingCrossSection(List<CeilingCrossSectionDataDTO> sectionDataDTOList, Product product) {
		if (null != sectionDataDTOList && sectionDataDTOList.size() > 0
				&& StringUtils.isNotBlank(sectionDataDTOList.get(0).getCeilingInfo())
				&& StringUtils.isNotBlank(sectionDataDTOList.get(0).getApplyArea())) {

			List<String> applyAreaStr = new ArrayList<>();
			for (CeilingCrossSectionDataDTO ceilingCrossSectionDataDTO : sectionDataDTOList) {
				String applyArea = ceilingCrossSectionDataDTO.getApplyArea();
				if (StringUtils.isNotBlank(applyArea)) {
					String[] str = applyArea.split(",");
					applyAreaStr.addAll(Arrays.asList(str));
				}

			}
			if (applyAreaStr.size() == 0) {
				throw new IllegalStateException("适用面积为空");
			}
			List<Dictionary> sysDictionaryList = dictionaryService.listByType("productCeilingApplyArea");

			if (sectionDataDTOList == null || sysDictionaryList.size() == 0) {
				throw new IllegalStateException("未从数据字典获取到适用面积");
			}

//			if (applyAreaStr.size() != sysDictionaryList.size()) {
//				throw new IllegalStateException("请补全截面天花的适用面积");
//			}


			String s = gson.toJson(sectionDataDTOList);
			product.setCeilingCrossSectionData(s);
		}
	}

	Integer handlerProCountertops(ProCtopsBO proCtopsBO, String splitTexturesInfo, Long productId) {
		if (proCtopsBO == null || productId == null || null == proCtopsBO.getDataTypeForOut() || null == proCtopsBO.getDataTypeForWall()) {
			return -1;
		}

		String textureType = "1";
		// 普通材质处理
		if (Objects.equals(textureType, "0")) {
			if (proCtopsBO.getIdForOut() != null || proCtopsBO.getIdForWall() != null) {
				// delete
				return baseProductCountertopsService.deleteProCtops(Arrays.asList(proCtopsBO.getIdForOut(), proCtopsBO.getIdForWall()));
			}
		...
		JSONObject defValue = null;
		if (!StringUtils.isEmpty(splitTexturesInfo)) {
			JSONArray textures = JSON.parseArray(splitTexturesInfo);
			if (textures != null && !textures.isEmpty()) {
				defValue = textures.getJSONObject(0);
			}
		}

		List<BaseProductCountertops> addCtops = new ArrayList<>();
		List<BaseProductCountertops> updateCtops = new ArrayList<>();

		// for wall
		BaseProductCountertops proCtopsForWall = new BaseProductCountertops();
		proCtopsForWall.setProductId(productId);
		proCtopsForWall.setTextureName(proCtopsBO.getTextureNameForWall());
		proCtopsForWall.setCrossSectionData(proCtopsBO.getCrossSectionDataForWall());
		proCtopsForWall.setDataType(Short.parseShort(Objects.toString(proCtopsBO.getDataTypeForWall(), "-1")));
		proCtopsForWall.setTextureId(Long.parseLong(Objects.toString(proCtopsBO.getTextureIdForWall(), "-1")));
		proCtopsForWall.setId(proCtopsBO.getIdForWall() == null ? null : proCtopsBO.getIdForWall().longValue());

		// for out
		BaseProductCountertops proCtopsForOut = new BaseProductCountertops();
		proCtopsForOut.setProductId(productId);
		proCtopsForOut.setTextureName(proCtopsBO.getTextureNameForOut());
		proCtopsForOut.setCrossSectionData(proCtopsBO.getCrossSectionDataForOut());
		proCtopsForOut.setDataType(Short.parseShort(Objects.toString(proCtopsBO.getDataTypeForOut(), "-1")));
		proCtopsForOut.setTextureId(Long.parseLong(Objects.toString(proCtopsBO.getTextureIdForOut(), "-1")));
		proCtopsForOut.setId(proCtopsBO.getIdForOut() == null ? null : proCtopsBO.getIdForOut().longValue());

		if (proCtopsForWall.getId() == null) {
			addCtops.add(proCtopsForWall);
		} else {
			updateCtops.add(proCtopsForWall);
		}

		if (proCtopsForOut.getId() == null) {
			addCtops.add(proCtopsForOut);
		} else {
			updateCtops.add(proCtopsForOut);
		}


		/**
		 * 1、台面选择了材质后，两个挡水边的材质自动默认为台面的材质，同时可以单独修改各个挡水边的材质；
		 * 2、若台面为可拆分产品，挡水边的材质跟随台面第一个材质变化； 3、挡水边的截面数据为一段顶点坐标信息。
		 */
		Long defTextureId = defValue != null ? defValue.getLong("defaultId") : -1;
		String defTextureName = defValue != null ? defValue.getString("name") : "";

		if (proCtopsForWall.getTextureId() == null || proCtopsForWall.getTextureId() == -1) {
			proCtopsForWall.setTextureId(defTextureId);
			proCtopsForWall.setTextureName(defTextureName);
		}

		if (proCtopsForOut.getTextureId() == null || proCtopsForOut.getTextureId() == -1) {
			if (proCtopsForWall.getTextureId() != null && proCtopsForWall.getTextureId() != -1) {
				proCtopsForOut.setTextureId(proCtopsForWall.getTextureId());
				proCtopsForOut.setTextureName(proCtopsForWall.getTextureName());
			} else {
				proCtopsForOut.setTextureId(defTextureId);
				proCtopsForOut.setTextureName(defTextureName);
			}
		}
		...
		if (!addCtops.isEmpty()) {
			// add
			updateCount += baseProductCountertopsService.addProCtops(addCtops);
		}
		if (!updateCtops.isEmpty()) {
			// update
			updateCount += baseProductCountertopsService.updateProCtops(updateCtops);
		}
    ...
    private List<Long> syncSoftProductSKUInfoAndSave(Product productSource, ProductAdd add) {
		...
		products.forEach(product -> {
			// 处理橱柜台面产品数据
			this.handlerProCountertops(add.getProCtopsBO(), product.getSplitTexturesInfo(), product.getId());
...
//		}
    ...
    private Product setProductInfos(ProductAdd add) {
        ...
        product.setDecorationPrice(add.getDecorationPrice());
        product.setIsSpecialEffects(add.getIsSpecialEffects());
        if(StringUtils.isNotBlank(add.getBedsideDimensions())) {
            int num = new Double(Double.parseDouble(add.getBedsideDimensions()) * 100).intValue();
            product.setBedsideDimensions(num);
        }
...
//			}
		...
		/*天花截面数据处理*/
		this.dealWithCeilingCrossSection(add.getCeilingCrossSectionDataDTOList(), product);
    ...
    private void syncProductRelInfos(ProductAdd add, List<Product> products) {
        ...
        // 插入分类信息
        if (Objects.nonNull(add.getCategoryIds())) {
			//瓷砖、大理石、木地板分类自动挂载到地面地砖、地面石材、地板和墙面墙砖、墙面石材、墙板
			List<Integer> categoryIds = autoMountProductCategory(add.getCategoryIds(), add.getProductSmallType());
			updateCategoryAttrInfo(categoryIds, productIds);
        }
        // 关联产品属性信息
	...
	//瓷砖、大理石、木地板在选了自身分类的同时自动挂载到地面地砖、地面石材、地板和墙面墙砖、墙面石材、墙板分类
	private List<Integer> autoMountProductCategory(List<Integer> categoryIds, String productSmallType) {
		//转换set去重
		Set<Integer> set = new HashSet<>(categoryIds);
		//获取大小分类对象
		Map<String, Dictionary> typeInfo = setProductTypeInfo(productSmallType, new Product());
		if (null == typeInfo || typeInfo.size() == 0) {
			return categoryIds;
		}
		//判断是否需要自动挂载分类(瓷砖、大理石、木地板)
		if (Objects.equals(Dictionary.PRODUCT_TYPE_KEY_CZ, typeInfo.get(Dictionary.DIC_TYPE_TYPE).getValuekey())
				|| Objects.equals(Dictionary.PRODUCT_TYPE_KEY_DALS, typeInfo.get(Dictionary.DIC_TYPE_TYPE).getValuekey())
				|| Objects.equals(Dictionary.PRODUCT_TYPE_KEY_DB, typeInfo.get(Dictionary.DIC_TYPE_TYPE).getValuekey())) {
			List<Category> categoryList = categoryService.getCategoryByIds(categoryIds);
			categoryList.forEach(category -> {
				//瓷砖自动挂到地砖、墙砖下
				if (Dictionary.PRODUCT_TYPE_KEY_CZ.equals(category.getThirdStageCode())) {
					// 地砖
					Integer categoryId = categoryService.getIdByNameOrFourCode(category.getName(), Dictionary.PRODUCT_SMALL_TYPE_KEY_DIZ);
					if (null != categoryId) {
						set.add(categoryId);
					}
					// 墙砖
					categoryId = categoryService.getIdByNameOrFourCode(category.getName(), Dictionary.PRODUCT_SMALL_TYPE_KEY_QIANGZ);
					if (null != categoryId) {
						set.add(categoryId);
					}
				}
				//大理石自动挂载到地面石材、墙面石材
				if (Dictionary.PRODUCT_TYPE_KEY_DALS.equals(category.getThirdStageCode())) {
					// 地面石材
					Integer categoryId = categoryService.getIdByNameOrFourCode(category.getName(), Dictionary.PRODUCT_SMALL_TYPE_KEY_DIS);
					if (null != categoryId) {
						set.add(categoryId);
					}
					// 墙面石材
					categoryId = categoryService.getIdByNameOrFourCode(category.getName(), Dictionary.PRODUCT_SMALL_TYPE_KEY_QIANGS);
					if (null != categoryId) {
						set.add(categoryId);
					}
				}
				//木地板自动挂载到地板、墙板
				if (Dictionary.PRODUCT_TYPE_KEY_DB.equals(category.getThirdStageCode())) {
					// 地板
					Integer categoryId = categoryService.getIdByNameOrFourCode(category.getName(), Dictionary.PRODUCT_SMALL_TYPE_KEY_DIB);
					if (null != categoryId) {
						set.add(categoryId);
					}
					// 墙板
					categoryId = categoryService.getIdByNameOrFourCode(category.getName(), Dictionary.PRODUCT_SMALL_TYPE_KEY_QIANGB);
					if (null != categoryId) {
						set.add(categoryId);
					}
				}
    ...
    private void setProductModelOrTexture(
        ...
        if (Category.CATEGORY_TYPE_MODEL.equals(Integer.parseInt(smallType.getAtt1Info()))
            ...
            if (modelInfos == null) {
				...
				if (product.getWindowsU3dmodelId() > 0) {
					//三度后台模型, model_area中查不到区域材质信息，此处不更新
				} else {
					//注释原因： 三度后台存在无模型，但是有spiltTexturesInfo　的数据，此处制空会导致这批数据有问题。
//					product.setSplitTexturesInfo("");
                    ...
                    area -> {
                        ...
                        if (textureIdsStr.length() > 0) {
                            ...
                            if (pti.getDefaultId() == null) {
                                ...
                                list.stream()
										.filter(areaTextureRel ->
												areaTextureRel.getAreaId().equals(area.getId()) && areaTextureRel.getTextureId().equals(areaDefaultTextureId))
    ...
    public boolean updateProductBiz(ProductUpdatePO update) {
        ...
        log.error("更新产品参数=" + new Gson().toJson(product));
        // 内容库才可编辑以下信息
        if (Platform.PLATFORM_LIBRARY.equals(update.getEditType())) {
			...
			// 处理橱柜台面产品数据
			this.handlerProCountertops(update.getProCtopsBO(), product.getSplitTexturesInfo(), product.getId());

			/*天花截面数据处理*/
			this.dealWithCeilingCrossSection(update.getCeilingCrossSectionDataDTOList(), product);

            log.error("更新产品参数=" + new Gson().toJson(product));
            // 更新产品
            productService.updateProduct(product);
			productService.updateProductOptionalParam(product);
			...
			//瓷砖、大理石、木地板分类自动挂载到地面地砖、地面石材、地板和墙面墙砖、墙面石材、墙板
			List<Integer> categoryIds = autoMountProductCategory(update.getCategoryIds(), update.getCategoryCode());

            // 更新产品运营树
			updateCategoryAttrInfo(categoryIds, Collections.singletonList(update.getId().intValue()));
    ...
    private Product setProductBaseInfo(ProductUpdatePO update, @NotNull Product product) {
        ...
        //床沿尺寸
        if(StringUtils.isNotBlank(update.getBedsideDimensions())){
            int num = (int)(new Double(update.getBedsideDimensions()) * 100);
            product.setBedsideDimensions(num);
        }
        product.setDecorationPrice(update.getDecorationPrice());
        product.setIsSpecialEffects(update.getIsSpecialEffects() == null ? 0 : update.getIsSpecialEffects());
    ...
    public List<Integer> updateHardProductBiz(HardProductUpdatePO updatePO) {
		...
		if (mainProductPO.getModelTextureInfos().isEmpty()) {
			//处理三度后台的产品,因为三度后台的产品模型，编辑时不会带过来材质信息
			mainProductPO.setModelTextureInfos(null);
		}
        // 更新
                    ...
                    () -> {
                        try {
                            ...
                            skuInfo
                                    ...
                                    .forEach(
											sku -> this.syncHardProductBaseInfoWithMainProduct(sku, mainProduct, smallType)
									);
                            log.info(
    ...
    /**
     * 根据硬装主产品,更新其它产品基本信息 以及模型信息 若manProduct中的产品ID不存在,则必然为新增硬装主产品
     * 若mainProduct的ID存在,则为同步副产品
     *
     * @param skuInfo
     * @param mainProduct
     * @param smallType
    ...
    private void setHardProductSizeWithModelSize(@NotNull Integer modelId, Product product) {
		...
		if (!detail.getModelCode().startsWith("M") && detail.getModelCode().contains("_")) {
			// 三度后台产品规格不处理
			product.setProductLength(null);
			product.setProductWidth(null);
			product.setProductHeight(null);
			product.setSplitTexturesInfo(null);
			product.setMaterialPicIds(null);
		} else {
    ...
    public ProductBO findProductInfoByProductId(long productId, String platformType) {
		...
		//合并产品标识
		Integer mergeFlag = productService.getProductMergeFlagByProductId(productId);
		if (Objects.equals(mergeFlag, 1)) {
			this.setMergeProducts(ret);
			return ret;
		}
	...
	private void setMergeProducts(ProductBO ret) {
		List<MergeProductBO> mergeProductBOS = productService.getMergeProductsByMainProductId(ret.getId());
		ret.setMergeProducts(mergeProductBOS);
	}
...
//														.sum());
    ...
    private void setProductDiyViewInfo(int productId, Product product, ProductBO ret) {
        ...
        // 设置产品装修单位
        ret.setValuationUnit("元/个");
        if (StringUtils.isNotBlank(product.getProductSmallTypeMark())) {
            ...
            if (null != productUnitPrices && productUnitPrices.size() > 0) {
                ...
                for (Dictionary productUnitPrice : productUnitPrices) {
                    ...
                    if (StringUtils.isNotBlank(productUnitPrice.getAtt3Info()) && productUnitPrice.getAtt3Info().contains(product.getProductSmallTypeMark())) {
                        ret.setValuationUnit(productUnitPrice.getName());
                    }
                }
            }
        }
                ...
                categoryService
                        ...
                        .map(
                                category -> {
									...
									//页面展示第二级
									Category category1 = categoryService.getCategoryByCodeOrLevel(category.getThirdStageCode(), 2);
									if (null != category1) {
										node.setSecondLevelName(category1.getName());
										node.setSecondLevelId(category1.getId().intValue());
									}
                                    return node;
                                })
                        .collect(Collectors.toList());
        ...
        // 设置产品特效信息
        ProductExtension productExtension = productDao.getProductExtensionByProductId(productId);
        if (null != productExtension) {
            ret.setIsSpecialEffects(productExtension.getIsSpecialEffects());
        } else {
            ret.setIsSpecialEffects(ProductExtension.PRODUCTS_NO_HAVE_SPECIAL_EFFECTS);
        }
        ...
        // 设置床沿尺寸
		if (null != product.getBedsideDimensions()) {
            ret.setBedsideDimensions(new Double(product.getBedsideDimensions()) / 100);
		}
		if (StringUtils.isNotBlank(product.getProductTypeValue()) && 9 == Integer.parseInt(product.getProductTypeValue())) {
			ret.setIsBedsideDimensions(1);
		}
		// xxx : 处理天花截面、挡水边材质
		// create by zhoujc at  2019/3/5 10:59.
		if (StringUtils.isNotEmpty(product.getCeilingCrossSectionData())) {
			List<CeilingCrossSectionDataDTO> crossSectionDataDTOList = gson.fromJson(product.getCeilingCrossSectionData(),
					new TypeToken<List<CeilingCrossSectionDataDTO>>() {
					}.getType());
			List<Dictionary> productCeilingApplyArea = dictionaryService.listByType("productCeilingApplyArea");
			for (CeilingCrossSectionDataDTO it : crossSectionDataDTOList) {
				List<String> itAreas = Arrays.asList(Strings.nullToEmpty(it.getApplyArea()).split(Punctuation.COMMA));
				String itAreaValues = productCeilingApplyArea.stream()
						.filter(t -> itAreas.contains(t.getName()))
						.map(t -> t.getValue() + "")
						.collect(Collectors.joining(Punctuation.COMMA));
				it.setApplyAreaValue(itAreaValues);
			}
			ret.setCeilingCrossSectionDataDTOList(crossSectionDataDTOList);
		}

		for (BaseProductCountertops it : baseProductCountertopsService.listProCtopsByProductId(product.getId())) {
			if (ret.getProCtopsBO() == null) {
				ret.setProCtopsBO(new ProCtopsBO());
			}
			ProCtopsBO proCtopsBO = ret.getProCtopsBO();
			if (Objects.equals(it.getDataType(), (short) 1)) {
				proCtopsBO.setIdForWall(it.getId().intValue());
				proCtopsBO.setDataTypeForWall(it.getDataType().intValue());
				proCtopsBO.setCrossSectionDataForWall(it.getCrossSectionData());
				proCtopsBO.setTextureIdForWall(it.getTextureId().intValue());
				proCtopsBO.setTextureNameForWall(it.getTextureName());
				proCtopsBO.setTexturePicPathForWall(it.getTexturePicPath());
			} else {
				proCtopsBO.setIdForOut(it.getId().intValue());
				proCtopsBO.setDataTypeForOut(it.getDataType().intValue());
				proCtopsBO.setCrossSectionDataForOut(it.getCrossSectionData());
				proCtopsBO.setTextureIdForOut(it.getTextureId().intValue());
				proCtopsBO.setTextureNameForOut(it.getTextureName());
				proCtopsBO.setTexturePicPathForOut(it.getTexturePicPath());
			}
		}
    ...
    public PageInfo<SolutionProductListBO> querySolutionProduct(ProductQueryPO po) {
        ...
        log.info("查询产品的信息:{}",productListBOS.getList());
        ...
        for (ProductListBO bo : productListBOS.getList()) {
            ...
            sbo.setDecorationPrice(bo.getDecorationPrice());
            list.add(sbo);
        }
    ...
    private PageInfo<ProductListBO> productListDoQuery(ProductQueryPO po) {
        ...
        if (po.getQueryType().equals(Platform.PLATFORM_LIBRARY) || po.getProductIds() != null) {
            ...
            List<Integer> ids =
					list.getList().stream().map(ProductListBO::getId).collect(Collectors.toList());
            ...
            for (ProductListBO bo : list.getList()) {
				...
				List<String> excludePlatforms = Platform.EXCLUDE_PLATFORMS;
				for (Integer id : platFormIds) {
					String name = platformMap.get(id);
					if (name == null) {
						continue;
					}
					if (excludePlatforms.contains(name)) {
						continue;
					}
					bo.getPutAwayPlatformIds().add(id);
					bo.getPutAwayPlatformNames().add(name);
				}
    ...
    private void setProductDiyViewInfo(List<ProductListBO> list) {
		...
		List<Integer> modelingIds = new ArrayList<>();
        for (ProductListBO tmp : list) {
			...
			if (tmp.getModelingId() != null) {
				modelingIds.add(tmp.getModelingId());
			}
        }
		...
		Map<Integer, String> styleMap = productStyleService.id2Name(list.stream().map(ProductListBO::getModelingId).filter(Objects::nonNull).collect(toList()));
        ...
        for (ProductListBO tmp : list) {
			// 设置产品款式
			tmp.setModelingName(styleMap.get(tmp.getModelingId()));
            // 设置产品的品牌名称
	...
	public List<Product> getSplitTexturesInfo() {
		return productService.getSplitTexturesInfo();
	}

	@Override
	public void processSplitTexturesInfo(List<Product> productList) {
		productList.forEach(product -> {
			try {
				log.info("开始处理产品异常数据,产品id:{},产品名称:{}",product.getId(),product.getProductName());
				List<ProductTextureInfo> productTextureInfos = new ArrayList<>();
			    Set<Integer> materialPicIds = new HashSet<>();
			    List<ModelAreaRel> list = modelAreaRelService.listByModelId(product.getWindowsU3dmodelId());
			    List<Integer> ids = list.stream().map(ModelAreaRel::getId).collect(toList());
			    List<ModelAreaTextureRel> areaTextureRels = modelAreaTextureRelService.getModelAreaTextureRelList(ids);
				list.forEach(area -> {
		            ProductTextureInfo pti = new ProductTextureInfo();
		            StringBuilder textureIds = new StringBuilder();
		            areaTextureRels.stream()
		                    .filter(rel -> rel.getAreaId().equals(area.getId()))
		                    .forEach(rel -> {
		                        rel.setAreaId(area.getId());
		                        pti.setName(area.getName());
		                        pti.setKey((Integer.valueOf(area.getCode())+1)+"");
		                        pti.setWidth(area.getWidth());
		                        pti.setHeight(area.getHeight());
		                        pti.setWidth(area.getWidth());
		                        pti.setHeight(area.getHeight());
		            ...
		            String textureIdsStr = "";
		            if (textureIds.length() > 1) {
		            	textureIdsStr = textureIds.substring(0, textureIds.length() - 1);
		            }else {
		            	textureIdsStr = textureIds.toString();
		            }
		            pti.setTextureIds(textureIdsStr);
		            if(pti.getDefaultId() != null){
		        ...
		        //updateProduct.setMaterialPicIds(Joiner.on(Punctuation.COMMA).skipNulls().join(materialPicIds));
		        updateProduct.setSplitTexturesInfo(JsonParser.toJson(productTextureInfos));
		        productService.updateProduct(updateProduct);
		        //做删除标记
		        productService.updateProductBak(product.getId(),1);
			}catch(Exception e) {
				log.info("处理产品异常数据失败,产品id:{},产品名称:{}",product.getId(),product.getProductName());
			}
		});
	}

	@Override
	public Integer syncProductInfoToGoods(List<Integer> productIds, Long userId) {
		...
		for (Product product : products) {
			...
			spu.setId(product.getGoodsSpuId());
			spu.setPicId(product.getPicId());
			spu.setPicIds(product.getPicIds());
			spu.setProductModelNumber(product.getProductModelNumber());
			spu.setProductDesc(product.getProductDesc());
			spu.setSpuName(product.getProductName());
			spu.setProductName(product.getProductName());
			spu.setGmtModified(LocalDateTime.now());
			spu.setModifier(userId + "");

			baseGoodsSpuService.updateGoods(spu);
		}
		return 1;
	}
	...
	public void mergeProducts(ProductMerge productMerge) {
		// check product type and brand
		if (!productService.checkMergeData(productMerge.getProductIds())) {
			throw new RuntimeException("合并产品分类、品牌必须相同...");
		}

		Integer preMainId = productMerge.getPreMainProductId();
		Integer curMainId = productMerge.getCurMainProductId();
		//设置缺省数据
		serProductMergeFromMainProduct(productMerge, curMainId);

		List<Integer> productIds = productMerge.getProductIds();
		Set<Integer> preProductIds = Collections.emptySet();

		if (preMainId != null) {
			productService.updateProductMergeFlag(preMainId, 0, productMerge.getLoginUser().getId());
			preProductIds = productService.listSecondProductId2ModelIdWithMainId(preMainId.longValue()).keySet();
		}

		//update main product
		productService.updateProductMergeFlag(curMainId, 1, productMerge.getLoginUser().getId());
		productMerge.setProductBatchType(2);
		productMerge.setProductIds(Collections.singletonList(curMainId));
		productService.mergeProducts(productMerge, productMerge.getLoginUser().getId());

		// update products
		productMerge.setProductBatchType(0);
		productMerge.setProductIds(productIds.stream().filter(it -> !it.equals(curMainId)).collect(toList()));
		productService.mergeProducts(productMerge, productMerge.getLoginUser().getId());

		// release canceled product
		List<Integer> canceledIds = preProductIds.stream().filter(it -> !productIds.contains(it)).collect(toList());
		productMerge.setProductBatchType(1);
		productMerge.setProductIds(canceledIds);
		productMerge.setCurMainProductId(0);
		productService.mergeProducts(productMerge, productMerge.getLoginUser().getId());
	}

	private void serProductMergeFromMainProduct(ProductMerge productMerge, Integer curMainId) {
		Product product = productService.getProductInfoById(curMainId);
		if (product == null) {
			throw new RuntimeException("获取主产品失败");
		}
		if (StringUtils.isBlank(productMerge.getProductName())) {
			productMerge.setProductName(product.getProductName());
		}
		if (StringUtils.isBlank(productMerge.getDefaultPicId()) && product.getPicId() != null) {
			productMerge.setDefaultPicId(product.getPicId() + "");
		}
		if (StringUtils.isBlank(productMerge.getModelNumber())) {
			productMerge.setModelNumber(product.getProductModelNumber());
		}
		if (StringUtils.isBlank(productMerge.getPicIds())) {
			productMerge.setPicIds(product.getPicIds());
		}
		if (productMerge.getSeriesId() == null) {
			productMerge.setSeriesId(product.getSeriesId());
		}
		if (productMerge.getModelingId() == null) {
			productMerge.setModelingId(product.getModelingId());
		}
	}


	public static void main(String[] args) {
		String s = "<p>2004年，深圳市壹丹天人合一装饰设计工程有限公司在充满着对优雅生活态度和艺术经典的美好敬意下诞生，致力于探索人与环境空间的协调关系 ，专注于室内及软装设计，为各位中高层面的客户项目提供最佳的现场视觉传达。十多年岁月中，逐渐磨劢成一家综合实力强劲的专业性设计及施工机构。</p><p><br></p><p>&nbsp;壹丹专注于各类开发商、公寓综合体及各类学校园区的建筑规划设计、品牌形象及广告设计、室内及软装设计与实施落地，主攻方向是：开发商的样板房、售楼处、会所及酒店公寓、学校等的品牌设计、室内、软装设计与实施，服务客户包括万科、龙湖、中海、恒大、金地、港中旅等地产公司，南山外国语学校,海丽达幼儿园,平遥示范幼儿园,行知小学,湖北云梦中学等学校，嘉汇新城酒店式产权公寓、麦浩斯、简客连锁公寓等酒店及公寓。</p><p><br></p><p>业务遍及北京、上海、广州、深圳、天津、西安、合肥、武汉、重庆、成都、杭州、昆明、长沙、郑州、长春、太原等全国各大、中型城市。十多年来始终秉承独特的设计理念，发扬特别关注细节的精神，用专业的技术水平及服务标准，一流的设计理念和丰富的案例操作经验为客户提供人性化、实用性、艺术性、超前性的高端设计产品及完美的服务。</p><p><br></p><p><br></p><p>&nbsp;壹丹公司将以稳健的经营、雄厚的实力、成熟的设计操作以及星级的服务，不断进取，努力发展成为行业内的顶级品牌。</p><p><br></p><p><img src=\"https://show.sanduspace.com/AA/c_companyshop/2019/5/30/14/company/shop/pic/small/thumbnial-27760292-small.jpg\"></p><p><br></p><p><img src=\"https://show.sanduspace.com/AA/c_companyshop/2019/5/30/14/company/shop/pic/small/thumbnial-27760298-small.jpg\"></p><p><br></p><p><img src=\"https://show.sanduspace.com/AA/c_companyshop/2019/5/30/14/company/shop/pic/small/thumbnial-27760300-small.jpg\"></p><p><br></p><p></p>";

		if (null == Integer.valueOf(1)) {

		}
		String s1 = s.replaceAll("<p><img(.+?)></p>", "");
		System.out.println(s1);
		List<String> collect = Splitter.on("<p>")
				.omitEmptyStrings()
				.splitToList(s)
				.stream()
				.collect(toList());
