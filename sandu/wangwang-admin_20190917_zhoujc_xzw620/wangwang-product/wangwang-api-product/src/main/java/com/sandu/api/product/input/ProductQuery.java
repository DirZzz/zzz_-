...
public class ProductQuery implements Serializable {
    ...
    @ApiModelProperty("产品类型（0:多模型副产品;1:单产品;2:多模型主产品）")
    @Range(min = 0, max = 2)
    private Integer productBatchType;

    @ApiModelProperty("产品合并功能 ,1 : 查询可合并产品")
    ...
    @ApiModelProperty(value = "排除查询的ids")
    private String excludeProductIds;

    @ApiModelProperty(value = "子产品编码")
    @Size(max = 20, message = "子产品编码过长")
    private String sonProductCode;

    @ApiModelProperty("是否关联模型/贴图（0：不是，1：是）")
    private Integer isModelTexture;
