...
public class DesignPlanRecommendedServiceImpl implements DesignPlanRecommendedService {
	...
	private Map<String, String> applySpaceMap;
    ...
    @Autowired
    private PlanDecoratePriceMapper
            planDecoratePriceMapper;
    ...
    public int deleteDesignPlanRecommended(long id) {
        ...
        // log: 2019/8/1 全屋方案子方案删除流程修改
        List<Integer> fullPlanIds = this.getFullPlanWhichContainsThis(id);
        fullHouseDesignPlanService.deletePlanByIds(fullPlanIds);
	...
	private synchronized void initSpaceApplyMap() {
		if (this.applySpaceMap == null) {
			List<Dictionary> list = dictionaryService.listApplySpaceAreas();
            this.applySpaceMap = list.stream().collect(Collectors.toMap(it -> it.getAtt3() + "_" + it.getValue(), Dictionary::getName));
		}
	}

	@Override
	public List<String> fetchPlanApplySpaceNames(String spaceName, String applySpaceAreaValues) {
		List<String> spaceValue = Stream.of(Strings.nullToEmpty(applySpaceAreaValues).split(Punctuation.COMMA)).filter(StringUtils::isNoneBlank).collect(Collectors.toList());
		if (this.applySpaceMap == null) {
			this.initSpaceApplyMap();
		}
		return spaceValue.stream().map(it -> this.applySpaceMap.get(spaceName + "_" + it)).collect(toList());
	}
    ...
    public DesignPlanBO getBaseInfo(Long id, Integer companyId) {
		...
		//设置空间类型名称
		Dictionary houseTypeBean = dictionaryService.getByTypeAndValue("houseType", designPlanBO.getSpaceTypeId());
		if (houseTypeBean != null) {
			designPlanBO.setSpaceTypeValueKey(houseTypeBean.getValuekey());
			designPlanBO.setSpaceTypeName(houseTypeBean.getName());
		}
        //处理视频图片
        if (designPlanBO.getCustomizeVideoFileId() != null && designPlanBO.getCustomizeVideoFileId() > 0) {
            ResFile file = resFileService.getById(designPlanBO.getCustomizeVideoFileId().longValue());
            designPlanBO.setCustomizeVideoFilePath(file == null ? null : file.getFilePath());
        }
...
//                platform2cMap.get(designPlanBO.getPlanId().intValue()).getAtt1());
		...
		//处理适用面积
		List<String> names = this.fetchPlanApplySpaceNames(designPlanBO.getSpaceTypeName(), designPlanBO.getApplySpaceAreas());
		designPlanBO.setApplyAreaNames(names);
		return designPlanBO;
    }
    ...
    public DesignPlanBO getBaseInfo(Long id) {
        ...
        //设置空间类型id
        designPlanBO.setSpaceTypeId(spaceTypeMap.get(Long.valueOf(designPlanBO.getSpaceCommonId())));
        designPlanBO.setSpaceCommonName(houseType.get(designPlanBO.getSpaceTypeId()));
        //查询方案品牌id对应的id集
        List<String> brandIdStrs = Splitter.on(COMMA)
                .omitEmptyStrings()
                .trimResults()
    ...
    public Long updateDesignPlanRecommended(DesignPlanUpdate designPlanUpdate) {
        DesignPlanRecommended designPlanRecommended = DesignPlanRecommended
                ...
                .applySpaceAreas(designPlanUpdate.getApplySpaceAreas())
                .isChanged(1)
                .customizeVideoFileId(designPlanUpdate.getCustomizeVideoFileId())
                .build();
        ...
        for (DecoratePriceInfo it : priceInfos) {
            ...
            for (Dictionary parent : decorateType) {
                if (it.getDecoratePriceTypeValue().equals(parent.getValue())) {
                    //选出ranges
                    List<Dictionary> ranges = dictionaryService.listByType(parent.getValuekey());
...
//                designPlan2bPlatformMapper.deleteByPlanId(Arrays.asList(id), Arrays.asList("1"));
    ...
    @Override
    public Map<Integer, List<Integer>> fixWithPlanDecoratePrice() {
        //获取之前方案报价
        List<PlanDecoratePrice> priceList = planDecoratePriceMapper.selectByIsDeleted();
        ...
        List<Dictionary> decorateType = dictionaryService.listByType("decorateType");
        Date date = new Date();
        for (PlanDecoratePrice price : priceList) {

            for (Dictionary parent : decorateType) {
                if (price.getDecoratePriceType().equals(parent.getValue())) {
                    //选出ranges
                    List<Dictionary> ranges = dictionaryService.listByType(parent.getValuekey());

                    ranges.sort(Comparator.comparing(Dictionary::getOrdering));
                    price.setDecoratePriceRange(ranges.get(ranges.size() - 1).getValue());
                    ranges.stream()
                            .filter(range -> this.choosePriceRangeWithPrice(range, price.getDecoratePrice()))
                            .findFirst()
                            .ifPresent(range -> price.setDecoratePriceRange(range.getValue()));
                }
            }
            price.setRemark("__修复报价数据__;");
            price.setGmtModified(date);
            planDecoratePriceMapper.updateByPrimaryKeySelective(price);
        }
        return priceList.stream().collect(Collectors.groupingBy(PlanDecoratePrice::getPlanType, Collectors.mapping(it -> {
            PlanDecoratePrice tmp = (PlanDecoratePrice) it;
            if (tmp.getPlanType().equals(SyncMessage.PLAN_TYPE_RECOMMENDED)) {
                return tmp.getPlanRecommendId();
            }
            return 0;
        }, toList())));
    }
	...
	@Override
    public Map<Integer, String> getPushStateByPlanIds(List<Integer> planIds) {

        Map<Integer, String> id2State = designPlanRecommendedMapper.getPushStateByPlanIds(planIds)
                .stream()
                .collect(toMap(it -> it.getId().intValue(), DesignPlanRecommended::getPlanCode));
    ...
    @Override
	public void deleteDesignPlanRecommendedSuperiorByPlanIds(List<Integer> planIds, Integer planType) {
        if (planIds == null || planIds.isEmpty()) {
            return;
        }
		designPlanRecommendedMapper.deleteDesignPlanRecommendedSuperiorByPlanIds(planIds, planType);
    }

    @Override
    public Integer getFirstShopId(Long id) {
        return designPlanRecommendedMapper.getFirstShopId(id);
    }
    ...
    private int updateCompanyShopDesignPlan(CompanyShopDesignPlanAdd companyShopDesignPlanAdd, String userName, List<CompanyShopDesignPlan> companyShopDesignPlanList) {
        ...
        if (res <= 0) {
            ...
            companyShopDesignPlan.setIsDeleted(0);
            companyShopDesignPlan.setPlanType(companyShopDesignPlanAdd.getPlanType());
            companyShopDesignPlanList.add(companyShopDesignPlan);
        }
        return res;
    }
    ...
    @Override
    public DesignPlanRecommended findRecommendedInfoById(Long id) {
        return designPlanRecommendedMapper.findRecommendedInfoById(id);
    }

    @Override
    public Integer selectShelfPlanCount(PlanRecommendedQuery planRecommendedQuery) {
        return designPlanRecommendedMapper.selectShelfPlanCount(planRecommendedQuery);
    }

    @Override
    public Integer selectPlanCount(PlanRecommendedQuery planRecommendedQuery) {
        return designPlanRecommendedMapper.selectPlanCount(planRecommendedQuery);
    }

    @Override
    public List<Integer> getFullPlanWhichContainsThis(long id) {
        return designPlanRecommendedMapper.getFullPlanWhichContainsThis(id);
    }

    @Override
    public PageInfo<FullHousePlanDetailExportBO> getFullHouseDetailsByPlanId(Integer planId, Integer page, Integer limit) {
		PageHelper.startPage(page, limit);
        List<Integer> fullHouseIds = this.getFullPlanWhichContainsThis(planId);
        PageInfo<Integer> pageInfo = new PageInfo<>(fullHouseIds);
        if(fullHouseIds.isEmpty()){
            return new PageInfo<>(Collections.emptyList());
        }

        PageInfo<FullHousePlanDetailExportBO> result = new PageInfo<>(designPlanRecommendedMapper.getFullHouseDetailsAllPlanDetailsByFullHouseIds(fullHouseIds));
    ...
    @Override
    public Map<Integer, Integer> planId2FullPlanCount(List<Long> planIds) {
        if (CollectionUtils.isEmpty(planIds)) {
            return Collections.emptyMap();
        }
        List<Map<String, Integer>> data = designPlanRecommendedMapper.planId2FullPlanCount(planIds);
        if (CollectionUtils.isEmpty(data)) {
            return Collections.emptyMap();
        }
		return data.stream().collect(toMap(it -> Integer.valueOf(it.get("id") + ""), it -> Integer.valueOf(it.get("count") + "")));
    }
