...
//        }
        ...
        if (Strings.nullToEmpty(resModelAdd.getModelPath()).toLowerCase().endsWith("assetbundle")) {
            resModel.setOldVersionPath(resModelAdd.getModelPath());
        }
        ...
        if (Strings.nullToEmpty(resModelAdd.getModelPath()).toLowerCase().endsWith("assetbundle")) {
            ...
            // 新增模型升级任务
            modelUpdateTask2019Dao.insertSelectiveTaskWait(ModelUpdateTask2019.builder()
...
//                .categoryNames(cateNames)
        ...
        if (StringUtils.isNoneBlank(resModelUpdate.getModelPath()) &&
            ...
            // 同步修改file3dupath
            if (null != resModelUpdate.getIsSyncFile3duPath() && 1 == resModelUpdate.getIsSyncFile3duPath()) {
                resModel.setFile3duPath(resModelUpdate.getModelPath());
            }
        ...
        if (StringUtils.isNotBlank(resModelUpdate.getOldVersionPath())&&
                resModelUpdate.getOldVersionPath().trim().toLowerCase().endsWith(".assetbundle")) {
            resModel.setOldVersionPath(resModelUpdate.getOldVersionPath());

            if (StringUtils.isBlank(resModelUpdate.getModelPath())) {

                // 同步修改file3dupath
                if (null != resModelUpdate.getIsSyncFile3duPath() && 1 == resModelUpdate.getIsSyncFile3duPath()) {
                    resModel.setFile3duPath(resModelUpdate.getOldVersionPath());
                }
