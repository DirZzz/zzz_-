...
public class BaseGoodsController {
    ...
    @ApiOperation(value = "批量导出，参数ids是要导出的商品id集合", response = ReturnData.class)
    @GetMapping("/exportGoodsSKUsData")
    public void exportGoodsSKUsData(String ids, Integer companyId, HttpServletResponse response) {
        if (null == companyId || null == ids || ids.equals("")){
            return;
        }
        List<Integer> idList = Splitter.on(",").omitEmptyStrings().trimResults().splitToList(ids)
                .stream().map(Integer::valueOf).collect(Collectors.toList());
        PutAwayBO putAwayBO = new PutAwayBO();
        putAwayBO.setCompanyId(companyId);
        putAwayBO.setIds(idList);
        List<GoodsSKUExportVo> goodsSKUsExportData = baseGoodsSKUService.getGoodsSKUsExportData(putAwayBO);
        if (isEmpty(goodsSKUsExportData)) {
            return;
        }
        String[] title={"编码","商品名称","商品型号", "原价", "优惠价", "商品主缩略图", "商品分类", "库存","上架情况","是否有模型/贴图"};
        String fileName = "商品数据导出.xls";
        String sheetName = "sheet1";
        HSSFWorkbook wb = ExportExcel.getHSSFWorkbook(sheetName, title, goodsSKUsExportData,null,basePath);
        //响应到客户端
        try {
    ...
    @GetMapping("/modifyGoodsSort")
    public Object modifyGoodsSort(Integer id, Integer sort){
        if (id ==  null){
            return ReturnData.builder().success(false).message("商品id不能为空");
        }
        // 设置默认值 add by huangsongbo 2019.09.06
        if (sort == null) {
        	sort = 0;
        }
        
        int row = baseGoodsSPUService.modifyGoodsSort(id,sort);
        if (row > 0){
            sycMessageDoSend(SyncMessage.ACTION_UPDATE, Collections.singletonList(id));
    ...
    @ApiOperation(value = "更新商品spu排序信息", response = ReturnData.class)
    @GetMapping("/sort/edit")
    public Object sortSpu(Integer spuId, Integer sort) {
        ReturnData<Object> data = ReturnData.builder();
        if (spuId == null || sort == null) {
            throw new RuntimeException("参数错误");
        }
        int i = baseGoodsSPUService.sortEdit(spuId, sort);
        return i > 0 ?
                data.code(SUCCESS).message("成功保存" + i + "条SKU信息")
                : data.code(ResponseEnum.NOT_CONTENT).message("保存失败...");
    }
    ...
    private void sycMessageDoSend(Integer messageAction, List<Integer> ids) {
        ...
        try {
            ...
            log.error("send message to sync goods info failed....");
        }
        log.info("finish sync message....,message : {}", message);
    }
