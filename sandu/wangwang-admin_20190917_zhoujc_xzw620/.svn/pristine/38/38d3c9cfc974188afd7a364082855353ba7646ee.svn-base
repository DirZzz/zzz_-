...
public interface DesignPlanRecommendedMapper {
    ...
    List<DesignPlanDeliverInfoBO> mapDeliverTimesByPlanId(@Param("planIds") List<Integer> planIds,
                                                          @Param("companyId") Integer companyId,
                                                          @Param("planType") Integer planType);

    Integer checkDesignPlanIsDeleted(@Param("planId") Long planId);

    Integer getVr720SingleId(@Param("planId") Integer planId);

    List<DesignPlanBO> managerPlanList(DesignPlanRecommendedQuery query);

    List<DesignPlanRecommended> getPushStateByPlanIds(@Param("planIds") List<Integer> planIds);

    void deleteDesignPlanRecommendedSuperiorByPlanIds(@Param("planIds") List<Integer> planIds, @Param("planType") Integer planType);

    Integer getFirstShopId(@Param("id") Long id);

    Integer fetchPlanSpaceTypeByPlanId(Long id);

    DesignPlanRecommended findRecommendedInfoById(Long id);

    List<DesignerUserKVDto> queryDesignerListByCompanyId(@Param("companyId")Integer companyId, @Param("recommendedType")Integer recommendedType);

    List<DesignPlanBO> selectListByPlands(@Param("planIds") List<Integer> planIds);


    Integer selectShelfPlanCount(PlanRecommendedQuery planRecommendedQuery);

    Integer selectPlanCount(PlanRecommendedQuery planRecommendedQuery);

    List<Integer> getFullPlanWhichContainsThis(@Param("id") long id);

    List<FullHousePlanDetailExportBO> getFullHouseDetailsAllPlanDetailsByFullHouseIds(@Param("fullHouseIds") List<Integer> fullHouseIds);

    List<Map<String, Integer>> planId2FullPlanCount(@Param("planIds") List<Long> planIds);
}