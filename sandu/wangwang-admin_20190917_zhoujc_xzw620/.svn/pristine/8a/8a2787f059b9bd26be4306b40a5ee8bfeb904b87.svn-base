...
public class DesignPlanBizServiceImpl implements DesignPlanBizService {
	...
	@Value(value = "${new.render.url}")
	private String NEW_RENDER_URL;
	...
	public PageInfo<SolutionProductListBO> listDesignPlanProducts(DesignPlanProductQuery query) {
		...
		if (isNotEmpty(designPlanProductBOS)) {
			...
			productQueryPO.setProductName(query.getProductName());
			productQueryPO.setProductCode(query.getProductCode());
			...
			if (isNotEmpty(designPlanProducts)) {
				for (SolutionProductListBO designPlanProduct : designPlanProducts.getList()) {
					...
					log.info("decorationPrice:{}",designPlanProduct.getDecorationPrice());
				}
			}
			log.info("返回结果:{}",designPlanProducts.getList());
	...
	@Override
	public PageInfo<DesignPlanBO> getDesignPlanlistByIds(List<Integer> planIds) {
		List<DesignPlanBO> designPlanBOS = designPlanRecommendedMapper.selectListByPlands(planIds);
        ...
        if (Optional.ofNullable(planIds).isPresent() && !planIds.isEmpty()) {
            List<DesignPlanDeliverInfoBO> listPlanDeliver = designPlanRecommendedService.mapDeliverTimesByPlanId(planIds, designPlanBOS.get(0).getCompanyId(), 1);
        ...
        for (DesignPlanBO designPlanBO:designPlanBOS ){
            ...
            if (listMapDeliver != null
                    && listMapDeliver.get(designPlanBO.getPlanId().intValue()) != null) {
                designPlanBO.setDelivers(listMapDeliver.get(designPlanBO.getPlanId().intValue()));
            } else {
                designPlanBO.setDelivers(new ArrayList<>(0));
            }
            if(null != designPlanBO.getDesignerId() && !designPlanBO.getDesignerId().equals("")){
                User user = userService.selectById(Long.parseLong(designPlanBO.getDesignerId()));
                designPlanBO.setDesigner(null != user ? user.getUserName() : "");
            }
        }
        return new PageInfo<>(designPlanBOS);
	}
	...
	private PageInfo<DesignPlanBO> storePlanList(DesignPlanRecommendedQuery query) {
		...
		if (query.getManagerSXW() != null && query.getManagerSXW()) {
			designPlanBOS = designPlanRecommendedMapper.managerPlanList(query);
		} else {
			designPlanBOS = designPlanRecommendedMapper.planStoreList(query);
		}
		...
		if (query.getManagerSXW() != null && query.getManagerSXW()) {
			...
			Map<String, Integer> collect = nodeInfoService.listNodeDetails(
					...
					NODE_TYPE_DESIGN_PLAN_RECOMMENDED,
					Arrays.asList(
			...
			designPlanBOS.stream().peek(it -> {
				if (it.getShopIn720Page() == 0) {
					it.setShopIn720Page(it.getShopId());
				}
				it.setShowInSXWIndexFlag(Math.abs(it.getShowInSXWIndexFlag() - 1));
			...
			}).filter(it -> it.getShopIn720Page() != 0 && !it.getShopIn720Page().equals(it.getShopId()))
					.forEach(it -> {
						//处理当前720页面中的店铺log
						CompanyShop shop = companyShopMapper.selectByPrimaryKey(it.getShopIn720Page().longValue());
						if (shop != null) {
							it.setShopName(shop.getShopName());
						}
					});
		}
		...
		if (designPlanRecommendedMapper.getPrimaryPlan(shelfPO.getPlanId()) != null) {
			...
			plans.forEach(
					plan -> {
						...
						designPlanRecommended.setPutawayTime(new Date());
						designPlanRecommendedMapper.updateByPrimaryKeySelective(designPlanRecommended);
					});
		}
...
//			}
			log.info("######################开始遍历文件夹下文件:{}####################",path);
			org.apache.commons.io.FileUtils.copyDirectory(new File(path), new File(localFilePath));
			log.info("######################结束遍历文件夹下文件:{}####################",path);
		...
		return designPlanDelivery
				...
				.map(
						companyId -> {
							...
							//交付(复制)的方案去掉上架时间
							tempDesignPlan.setPutawayTime(null);
										...
										() -> {
											...
											plans.forEach(
													plan -> {
														...
														//交付(复制)的方案去掉上架时间
														tempDesignPlanSon.setPutawayTime(null);
	...
	public void processBrandPoductImage(
		...
		// copy plan  product
		List<DesignPlanRecommendedProduct> products = designPlanRecommendedProductService.listByPlanId(oldDesignPlanId);
...
//		}
		...
		Map<Integer, List<DesignPlanDeliverInfoBO>> listMapDeliver = new HashMap<>();
		if (Optional.ofNullable(planIds).isPresent() && !planIds.isEmpty()) {
			List<DesignPlanDeliverInfoBO> listPlanDeliver = designPlanRecommendedService.mapDeliverTimesByPlanId(planIds, companyId, 1);
			listMapDeliver = listPlanDeliver.stream().collect(Collectors.groupingBy(DesignPlanDeliverInfoBO::getSourceId, Collectors.toList()));
		}
		...
		Map<Integer, Integer> planId2FullPlanCount = designPlanRecommendedService.planId2FullPlanCount(designPlanBOS.stream().map(DesignPlanBO::getPlanId).collect(toList()));

		for (DesignPlanBO designPlanBO : designPlanBOS) {
			log.info(
					"##############设置方案id:{}相关信息表开始###########################", designPlanBO.getPlanId());
			//是否存在全屋方案包含此方案
			Integer count = planId2FullPlanCount.get(designPlanBO.getPlanId().intValue());
			designPlanBO.setUsedInFullPlanCount(count == null ? 0 : count);
			...
			if (!Strings.isNullOrEmpty(designPlanBO.getPlatformIds())) {
				...
				for (String str : platformId) {
					if (!StringUtil.isEmpty(str) && platformMap.containsKey(Integer.valueOf(str))) {
						...
						String name = platformMap.get(Integer.valueOf(str));
...
//				designPlanBO.setDeliverStatus(deliverTimesMap.get(designPlanBO.getPlanId().intValue()));
			...
			//处理适用面积
			List<String> strings = designPlanRecommendedService.fetchPlanApplySpaceNames(designPlanBO.getSpaceTypeName(), designPlanBO.getApplySpaceAreas());
			designPlanBO.setApplyAreaNames(strings);
	...
	public void copyBrandProductImage(Integer oldPlanId, Integer targetCompanyId, Integer newPlanId) {
		...
		// copy plan  product
		List<DesignPlanRecommendedProduct> products = designPlanRecommendedProductService.listByPlanId(oldPlanId);
	...
	public Long copyDesignPlanToCompany2(CopyShareDesignPlanPO planPO) {
		...
		for (Integer planId : planPO.getSourceDesignPlanIds()) {
			...
			// 复制好的方案ID
			if (newPlanId == null) {
				newPlanId = tempDesignPlan.getId();
			}
						...
						() -> {
							...
							plans.forEach(
								plan -> {
									...
									tempDesignPlanSon.setPutawayTime(null);//不拷贝上架时间
									designPlanRecommendedMapper.insertSelective(tempDesignPlanSon);
									sb.append(tempDesignPlanSon.getId()).append(",");
									log.info(
											"##############分享方案{}开始###########################", plan.getId());
	...
	private void processImageRoamVideo(
		...
		try {
			...
			for (ResRenderPic resRenderPic : renderPics) {
				...
				// 获取渲染截图ID
				if ("渲染截图".equals(resRenderPic.getPicType()) && resRenderPic.getRenderingType() == 8) {
			...
			if (pictureList != null && pictureList.size() > 0) {
				for (ResRenderPic resRenderPic : pictureList) {
					DesignRenderRoam roamTemp = designRenderRoamService.getByScreenIdAndFileKey(Integer.parseInt(resRenderPic.getId()+""),"design.designPlan.render.roam.config");
	 ...
	 *
	 * (CMS-744 20190530 - 需要同步更新design_plan_recommended表上架时间putaway_time)
	 * @throws Exception
	 */
	@Override
	public Map<String, Object> putlibraryDesignPlan(
		...
		if (resultMapAll.containsKey("0")) {
			//更新上架时间
			DesignPlanRecommended recommended = DesignPlanRecommended.builder()
					.id(Long.parseLong(planId))
					.putawayTime(new Date())
					.build();
			designPlanRecommendedMapper.updateByPrimaryKeySelective(recommended);
		}
		return resultMapAll;
	}
...
//		String vr720Roam = designPlanRecommendedMapper.getUUID(renderPicId);
		...
		//新720效果图
		Integer vr720SingleId = designPlanRecommendedMapper.getVr720SingleId(planId);
		Integer vr720RoamId = designPlanRecommendedMapper.getVr720Roam(planId);
...
//		resultMap.put("vr720Roam", RENDER_URL + "/pages/vr720/vr720Roam.htm?code=" + vr720Roam);
		resultMap.put("vr720Single", NEW_RENDER_URL + "renderId=" + vr720SingleId
				+"&sceneType=1&planSourceType=3&hasChanged=0&qrCodeType=0");
		resultMap.put("vr720Roam", NEW_RENDER_URL + "renderId=" + vr720RoamId
				+"&sceneType=2&planSourceType=3&hasChanged=0&qrCodeType=0");
		return resultMap;
	}
	...
	@Override
	public boolean editPlanForManager(ManagerSXWPlanUpdate update) {
		DesignPlanRecommended plan = designPlanRecommendedMapper.getDesignById(update.getPlanId());
		Date now = new Date();
		...
		if (update.getShopIn720Page() != null) {
			plan.setPlanName(update.getPlanName());
			plan.setShowInSXWFlag(update.getShowInSXWFlag() ? 1 : 0);
			plan.setShopIn720Page(update.getShopIn720Page());
			plan.setSort(update.getSortInSXWIndex());
		...
		if (update.getShowInSXWIndexFlag() != null) {
			//首页展示，首页排序
			List<DesignPlanRecommendedSuperior> list = designPlanRecommendedSuperiorMapper.ListByPlanIdAndSpaceType(update.getPlanId());
			Optional<DesignPlanRecommendedSuperior> first = list.stream().filter(it -> it.getDesignPlanRecommendedId().equals(update.getPlanId()))
					.findFirst();
			if (first.isPresent()) {
				// update 
				...
				if (list.size() >= 5) {
					throw new IllegalStateException("当前空间类型推荐方案数量为：" + list.size());
				}
				...
				// insert
				DesignPlanRecommendedSuperior tmp = list.isEmpty() ? new DesignPlanRecommendedSuperior() : list.get(0);
				tmp.setId(null);
				...
				tmp.setPlanName(plan.getPlanName());
				tmp.setSpaceType(spaceType);
				tmp.setPlanCode(plan.getPlanCode());
	...
	@Override
	public List<DesignerUserKVDto> queryDesignerListByCompanyId(Integer companyId, Integer recommendedType) {
		List<DesignerUserKVDto> users = designPlanRecommendedMapper.queryDesignerListByCompanyId(companyId, recommendedType);
