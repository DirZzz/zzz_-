...
public class ResTextureBizServiceImpl implements ResTextureBizService {
    ...
    public ResTextureVO getResTextureDetail(long id) {
        ...
        Optional.ofNullable(res.getClassification()).ifPresent(classification -> {
            Dictionary dictionary = dictionaryService.getByTypeAndValue("classification",res.getClassification());
            Optional.ofNullable(dictionary).ifPresent(item -> vo.setClassificationName(dictionary.getName()));
        });
        Optional.ofNullable(res.getColorSystem()).ifPresent(colorSystem -> {
            Dictionary dictionary = dictionaryService.getByTypeAndValue("colorSystem",res.getColorSystem());
            Optional.ofNullable(dictionary).ifPresent(item -> vo.setColorSystemName(dictionary.getName()));
        });
