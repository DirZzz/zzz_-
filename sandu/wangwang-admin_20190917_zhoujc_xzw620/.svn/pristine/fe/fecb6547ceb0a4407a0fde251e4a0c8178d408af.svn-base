...
import com.sandu.api.user.output.UserKVDto;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public List<UserKVDto> queryDesignerList() {
        List<Dictionary> dictionary = dictionaryMapper.getByTypeAndName("userType","设计师");
        ...
        List<UserKVDto> dtos = users.stream().map(it -> new UserKVDto()).collect(Collectors.toList());
        for (int i = 0; i < users.size(); i++) {
            BeanUtils.copyProperties(users.get(i), dtos.get(i));
            if(isEmpty(dtos.get(i).getUserName())){
                dtos.get(i).setUserName("");
            }
        }
        return dtos;
    }
