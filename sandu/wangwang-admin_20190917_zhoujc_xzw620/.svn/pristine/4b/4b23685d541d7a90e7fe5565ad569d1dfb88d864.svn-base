...
public class ResModelController extends BaseController {
   ...
   private Map<String,Object> checkResModel(Integer companyId,String modelCode) {
    ...
    public ReturnData saveResModel(@Valid @RequestBody ResModelUpdate resModelUpdate, BindingResult br) {
        ...
        if (!Strings.nullToEmpty(resModelUpdate.getModelPath()).equals(detail.getFile3duPath())) {
            ...
            if (Strings.nullToEmpty(resModelUpdate.getModelPath()).toLowerCase().endsWith("assetbundle")) {
                ...
                // 同时修改file3dupath
                resModelUpdate.setIsSyncFile3duPath(1);

                // 同时写入旧版本路径
                resModelUpdate.setOldVersionPath(resModelUpdate.getModelPath());

                // 如果当前模型为2019模型，不修改新模型路径，让模型升级任务修改
                if (detail.getModelPath().startsWith("/AA/model/resource")) {
                    resModelUpdate.setModelPath(null);
                }
    ...
    public ReturnData transFail(@RequestParam Long modelId) {
        updateModel(modelId, null, TransStatus.FAIL.getCode(),null);
    ...
    public ReturnData<FileVO> u3dUpload(
        ...
        if (file == null) {
            ...
            /* 修改数据库*/
            updateModel(modelId, filename, TransStatus.SUCCESS.getCode(),detail);
            FileVO vo = new FileVO();
    ...
    private void updateModel(Long modelId, String path, String transStatus,ResModel detail) {
        ...
        if (StringUtils.isNoneBlank(path)) {
            ...
            // 同时写入旧模型路径
            resModel.setOldVersionPath(path);

            // 如果当前模型为2019模型，不修改新模型，让模型升级任务修改
            if (StringUtils.isNotBlank(detail.getModelPath()) && detail.getModelPath().startsWith("/AA/model/resource")) {
            } else {
