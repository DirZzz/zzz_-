...
//            	}
        ...
        //再重新插入
        List<ModelAreaTextureRel> newAreaTextureRels = this.updateModelAreaRelAndDefaultTextureWithBalls(list, balls, areaTextureMap);

        //关联产品 材质信息
        ResModel resModelDetail = resModelService.getResModelDetail(modelId.longValue());
        if (resModelDetail.getProductId() != null && resModelDetail.getProductId() > 0) {
            Product product = productService.getProductInfoById(resModelDetail.getProductId());
            Product modelInfoProduct = this.insetModelAreaRelAndDefaultTextureWithTextures(reList, newAreaTextureRels);
            product.setMaterialPicIds(modelInfoProduct.getMaterialPicIds());
            product.setSplitTexturesInfo(modelInfoProduct.getSplitTexturesInfo());
    ...
    public List<ModelAreaTextureRel> updateModelAreaRelAndDefaultTextureWithBalls(List<ModelAreaRel> list, List<ResModelAdd> balls, Map<Integer, Integer> areaTextureMap) {
        if (list.isEmpty()) {
        ...
        list.forEach(item ->{
        	if(item.getId() == null) {
        		...
        		item.setGmtModified(new Date());
        		modelAreaRelService.updateModelAreaRel(item);
        	}
        });
        //关联区域-默认贴图
