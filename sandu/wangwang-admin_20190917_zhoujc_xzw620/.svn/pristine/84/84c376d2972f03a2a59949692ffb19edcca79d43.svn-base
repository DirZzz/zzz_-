...
public interface DesignPlanRecommendedService {
     ...
     * @param applySpaceAreaValues design_plan_recommended.apply_space_areas
     * @return 适用面积名称
     */
    List<String> fetchPlanApplySpaceNames(String spaceName, String applySpaceAreaValues);
    /**
     * 根据id查询数据
     *
     * @param id 主键
    ...
    Map<Integer, List<Integer>> fixWithPlanDecoratePrice();

    Map<Integer, String> getPushStateByPlanIds(List<Integer> planIds);

    void deleteDesignPlanRecommendedSuperiorByPlanIds(List<Integer> planIds, Integer planType);

    Integer getFirstShopId(Long id);
    ...
    List<Integer> getFullPlanWhichContainsThis(long id);

    PageInfo<FullHousePlanDetailExportBO> getFullHouseDetailsByPlanId(Integer planId, Integer page, Integer limit);

    Map<Integer, Integer> planId2FullPlanCount(List<Long> collect);
}
