...
public class DesignPlanController extends BaseController {
    ...
    public ReturnData listOneKeyDesignPlan(DesignPlanRecommendedQuery query) {
    	if(StringUtils.isNotEmpty(query.getPlatformId())) {
    		if("0".equals(query.getPlatformId())) {
                query.setPlatformBussinessType("NONE");
    		}else if(ShelfStatus.ONEKEY.getCode().equals(query.getPlatformId())||
    				ShelfStatus.OPEN.getCode().equals(query.getPlatformId())||
    				ShelfStatus.TEMPLATE.getCode().equals(query.getPlatformId())) {
    			query.setShelfStatus(query.getPlatformId());
    		}else {
    			query.setPlatformBussinessType(platformService.getPlatformBussinessTypeById(query.getPlatformId()));
    		}
    	}
    ...
    @ApiOperation(value = "一键方案列表导出")
    @GetMapping("/onekey/exportListExcel")
    public void exporListOneKeyDesignPlan(String planIds,HttpServletResponse response) {
        List<Integer> planIdList = Splitter.on(",").omitEmptyStrings().trimResults().splitToList(planIds)
                .stream().map(Integer::valueOf).collect(Collectors.toList());
        PageInfo<DesignPlanBO> datas = designPlanBizService.getDesignPlanlistByIds(planIdList);
        if (isEmpty(datas.getList())) {
            return;
        }
        List<DesignPlanExcelDto> dtos = datas.getList().stream().map(it -> new DesignPlanExcelDto()).collect(Collectors.toList());
        ListCopyUtils.copyListProps(datas.getList(), dtos);
        dtos.forEach(item -> {
            item.setCompleteDateStr(null != item.getCompleteDate() ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getCompleteDate()) : null);
            item.setPutawayTimeStr(null != item.getPutawayTime() ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getPutawayTime()) : null);
            item.setOrigin(getOriginName(item.getOrigin()));
            item.setDeliverName(getDeliverName(item.getDelivers()));
            item.setPicBytes(this.readFileToBytes(item));
        });
        createDesignPlanExcel(dtos, "一键方案列表导出",response);
    }
...
//            item.setPicPath("C:\\Users\\Sandu\\Desktop\\shamo.jpg");
    ...
    private void createDesignPlanExcel(List<DesignPlanExcelDto> list, String filename, HttpServletResponse response) {
        ...
        for (DesignPlanExcelDto project:list) {
            ...
            mapValue.put("designStyleName", project.getDesignStyleName());
            mapValue.put("solutionType", project.getSolutionType());
            mapValue.put("designer", project.getDesigner());
            mapValue.put("putawayTime", project.getPutawayTimeStr());
            mapValue.put("origin", project.getOrigin());
        ...
        String keys[] = {"planId", "coverPic", "planName", "spaceTypeName", "brandName", "designStyleName", "solutionType", "designer", "putawayTime", "origin", "deliverName", "completeDate"};
    ...
    @ApiOperation(value = "普通方案列表导出")
    @GetMapping("/common/exportListExcel")
    public void exportListCommonDesignPlan(String planIds,HttpServletResponse response) {
        List<Integer> planIdList = Splitter.on(",").omitEmptyStrings().trimResults().splitToList(planIds)
                .stream().map(Integer::valueOf).collect(Collectors.toList());
        PageInfo<DesignPlanBO> datas = designPlanBizService.getDesignPlanlistByIds(planIdList);
        if (isEmpty(datas.getList())) {
        ...
        ListCopyUtils.copyListProps(datas.getList(), dtos);
        dtos.forEach(item -> {
            item.setCompleteDateStr(new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getCompleteDate()));
            item.setPutawayTimeStr(null != item.getPutawayTime() ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(item.getPutawayTime()) : null);
            item.setOrigin(getOriginName(item.getOrigin()));
            item.setDeliverName(getDeliverName(item.getDelivers()));
            item.setPicBytes(this.readFileToBytes(item));
        });
        createDesignPlanExcel(dtos, "普通方案列表导出",response);
    }
...
*/
    ...
    public Object getDesignPlanRenderPic(@RequestParam(value = "planId") Integer planId) {
    ...
    public Object managerPlanInSXWIndex() {
...
//        Map<Integer, List<Integer>> type2Ids = designPlanRecommendedService.fixWithPlanDecoratePrice();
//        type2Ids.forEach((k, v) -> {
//            sycMessageDoSend(SyncMessage.ACTION_UPDATE, v, k);
//        });
        return "done";
    }

    @ApiOperation(value = "查询设计师列表", response = DesignerUserKVDto.class)
    @GetMapping(value = "/getDesignerList")
    public ReturnData getDesignerList(@RequestParam(value = "companyId") Integer companyId,@RequestParam(value = "recommendedType") Integer recommendedType) {
        if(null == companyId || null == recommendedType){
            return ReturnData.builder().code(PARAM_ERROR).message("参数为空");
        }

        List<DesignerUserKVDto> results = designPlanBizService.queryDesignerListByCompanyId(companyId, recommendedType);
    ...
    @ApiOperation(value = "获取使用此单空间的所有全屋方案详情", response = DesignerUserKVDto.class)
    @GetMapping(value = "/withFullPlan/singles")
	public ReturnData getFullHouseDetailByPlanId(@RequestParam(value = "planId") Integer planId, Integer page, Integer limit) {
        PageInfo<FullHousePlanDetailExportBO> details = designPlanRecommendedService.getFullHouseDetailsByPlanId(planId, page, limit);
        Collection<List<FullHousePlanDetailExportBO>> values = details.getList().stream().collect(Collectors.groupingBy(FullHousePlanDetailExportBO::getFullPlanId)).values();

        if (CollectionUtils.isNotEmpty(values)) {
            return ReturnData.builder().code(SUCCESS).data(values).total(details.getTotal());
        }
        return ReturnData.builder().code(NOT_CONTENT).message("暂无数据");
    }
