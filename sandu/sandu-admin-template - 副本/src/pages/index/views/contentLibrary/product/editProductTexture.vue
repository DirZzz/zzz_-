            ...
            <el-row>
              <el-col :span="12">
                <el-form-item label="一石多面产品：" label-width="120">
                  <el-select v-model="product.isComplexParquet" disabled style="width: 310px">
...
<script>
  ...
  export default {
    components: {ElRow,hardProduct,addHardProduct,setProductInfo,SingleImageUpload},
  ...
  methods: {
    ...
    handleSelect(item) {
      this.product.modelNumber=item.value;
    },
    ...
    getProduct() {
      this.API.ProductDetail({
        ...
        if (res) {
          ...
          this.getSelectSaleUnionName(res.data.smallType).then((filter)=> {
    ...
    getSelectSaleUnionName(smallType){
        return new Promise((resolve,reject)=>{
            let filter = this.unitList.filter(e => {
                if (e.suitableCodes)
                    return e.suitableCodes.indexOf(smallType) != -1;
            });
            resolve(filter)
        })
    },
        ...
        if (valid) {
          ...
          this.$confirm("确定更新产品", "提示", {
            ...
            .then(() => {
              ...
              let formDatass = {
                productUpdate: formDatas,
                resTextureUpdate: formTextureData
              };
              let jsonData = JSON.stringify(formDatass);
              console.log(this.addGroup, "this.addGroup ddd");
              if (sessionStorage.getItem("addProductType") == "soft") {
                //之前的逻辑 软装
                this.API.UpdateProductTexture(jsonData).then(res => {
                  if (res) {
