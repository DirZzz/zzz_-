    ...
    <page>
        <div class="main">
            <div class="main-header">
                <el-row>
                    <el-col :span="4">
                        <el-input v-model="formParam.companyCode" placeholder="请输入企业编码"></el-input>
                    </el-col>
                    <el-col :span="4">
                        <el-input v-model="formParam.companyName" placeholder="请输入企业名称"></el-input>
                    </el-col>
                    <el-col :span="4">
                ...
                <el-button type="primary" size="medium" round class="newAdd"
                           v-if="permission('manufacturer.manage.add')" @click="toLink('/userManage/mfrsManage/mfrsAdd')">
                    新增</el-button>
            </div>
                ...
                <el-table :data="tableData.list" v-loading="loading" element-loading-text="拼命加载中"
                    ...
                    <el-table-column prop="companyName" label="企业名称">
                        <template slot-scope="scope">
                            <span class="blues"
                                @click="toLink('/userManage/mfrsManage/mfrsInfo',{id:scope.row.id})">{{scope.row.companyName}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="320" align="center" label="操作">
                        <template slot-scope="scope">
                            <el-button type="text" v-if="permission('manufacturer.manage.update')"
                                @click="toLink('/userManage/mfrsManage/mfrsEdit',{id:scope.row.id})">编辑</el-button>
                            <el-button type="text" v-if="permission('manufacturer.manage.user')"
                                @click="toLink('/userManage/mfrsManage/userManage',{id:scope.row.id,businessType:scope.row.businessType})">
                                用户管理</el-button>
                            <el-button type="text" v-if="permission('manufacturer.manage.dealer')"
                                @click="toLink('/userManage/mfrsManage/dealerManage/' + scope.row.id)">经销商企业</el-button>
                            <el-button type="text" v-if="permission('manufacturer.manage.del')"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            industryChange(arr) {
                this.formParam.industryInfo = arr.industry;
            },
            search() {
                this.query.page = 1;
                this.comapnyList()
