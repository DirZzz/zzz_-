...
public class ResourceConfig {
    ...
    public static List<Map<String,String>> SERVER_ALL_URL;
    ...
    @Value("${server.all.url}")
    public void setServerAllUrl(String serverAllUrl) {
        if(StringUtils.isNotBlank(serverAllUrl)) {
            SERVER_ALL_URL = GsonUtil.fromJson(serverAllUrl,List.class);
        }
    }
     ...
     * @param userType      用户类型
     * @return
     * @create 2019/4/20 0020 14:46
     */
    public static List<Map<String,String>> getResourcesUrl(String  platformCode,Integer userType){

        List<Map<String,String>> resourcesUrlList = new ArrayList<>();

        if (null != SERVER_ALL_URL && SERVER_ALL_URL.size() > 0) {

            resourcesUrlList = SERVER_ALL_URL;
        }
