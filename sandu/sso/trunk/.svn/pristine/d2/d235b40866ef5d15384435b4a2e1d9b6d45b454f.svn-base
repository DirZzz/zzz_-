...
public interface SysUserDao {
    ...
    */
    ...
    List<UserManageDTO> findAllUserList(UserManageSearch search);

	List<Map<String,Object>> findUserPackageInfoByUserIds(@Param("userIds") List<Long> userIds);

	void updateFreezeState(SysUser sysUser);

    void batchUpdatePasswordByUserIds(@Param("initPassword")String password, @Param("password")String md5Password, @Param("userIds") List<Integer> userIds, @Param("modify") String loginName);

    List<SysUser> selectListByIds(@Param("userIds") List<Integer> userIds);

	Integer getMasterUserId(@Param("userId") Integer userId);

	List<UserMasterSonVO> selectCompanySonUserList(UserMasterSonSearch userMasterSonSearch);

	Integer selectUserMsTypeCount(@Param("idList") List<Integer> userIdList,@Param("companyId") Integer companyId,@Param("msType") Integer msType);

	Integer updateMasterSonType(@Param("idList") List<Integer> userIdList,@Param("masterSonType") Integer masterSonType);
