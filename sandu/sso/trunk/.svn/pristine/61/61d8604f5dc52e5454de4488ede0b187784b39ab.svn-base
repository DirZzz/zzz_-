...
public class SysUserManageServiceImpl implements SysUserManageService {
    ...
    public InternalUserDetailVO getInternalUserDetail(Long userId) {
        ...
        if (sysUser!=null){
            ...
            userDetail.setMasterSonType(sysUser.getMasterSonType());
        ...
        try{
            if (null!=userId&&null!=failureTime){
                ...
                if (null!=sysUser&&null!=sysUser.getFirstLoginTime()&&null==sysUser.getFailureTime()){
                    ...
                    /**
                     * modify by qta 20190624
                     * 没有激活账号的用户,会在更改套餐时直接添加log
                     */
                    if (CollectionUtils.isEmpty(sysUserOperatorLogService.getLogByUserId(userId))){
...
//        }
    ...
    @Override
    public Map<String, String> mangeFreeze(AccountFreezeManage manage) {
        Date now = new Date();
        Map<String, String> msgMag = new HashMap<>(1);
        List<String> failedUserList = new LinkedList<>();
        List<String> failedUserList2 = new LinkedList<>();
        String noFreezeFailedMsg = "登录名为：%s ，没有设置暂停使用时间，设置开启时间失败";
        String enableBeforeFreezeFailedMsg = "登录名为：%s ，设置开启时间小于暂停使用时间，设置失败";
        for (Integer id : manage.getUserIds()) {
            SysUser sysUser = sysUserDao.selectByPrimaryKey(id);
            if (sysUser == null) {
                continue;
            }
            sysUser.setId(id.longValue());
            if ("freeze".equals(manage.getState())) {
                sysUser.setAccountFreezeTime(manage.getDate());
                sysUser.setAccountEnableTime(null);
            }
            if ("enable".equals(manage.getState())) {
                if (sysUser.getAccountFreezeTime() == null) {
                    failedUserList.add(sysUser.getNickName());
                    continue;
                }
				if (manage.getDate() != null && sysUser.getAccountFreezeTime().after(manage.getDate())) {
                    failedUserList2.add(sysUser.getNickName());
                    continue;
                }
                sysUser.setAccountEnableTime(manage.getDate());
            }
            sysUser.setGmtModified(now);
            sysUser.setModifier(manage.getLoginUser().getLoginName());
            sysUserService.updateFreezeState(sysUser);
            this.logAccountStateAction(id, manage.getState(), manage.getDate(), manage.getLoginUser().getId());
        }
        if (failedUserList.size() > 0) {
            msgMag.put(noFreezeFailedMsg, Joiner.on(Punctuation.COMMA).join(failedUserList));
        }
		if (failedUserList2.size() > 0) {
			msgMag.put(enableBeforeFreezeFailedMsg, Joiner.on(Punctuation.COMMA).join(failedUserList2));
		}
        return msgMag;
    }

    private void logAccountStateAction(Integer userId, String state, Date date, Long modiferId) {
        ServiceActionDetailsLog log = new ServiceActionDetailsLog();
        String msg = "freeze".equals(state) ? AccountFreezeManage.SERVICE_ACTION_LOG_MSG_FREEZE : AccountFreezeManage.SERVICE_ACTION_LOG_MSG_ENABLE;
        log.setActionType(3);
        log.setRemark(String.format(msg, modiferId, date));
        log.setGmtCreate(new Date());
        log.setCreator(modiferId.toString());

        servicesPurchaseBizService.insertServiceActionLog(log);
    }
    ...
    @Override
    public PageInfo<UserMasterSonVO> masterSonUserList(UserMasterSonSearch search){

        com.github.pagehelper.PageHelper.startPage(search.getPage(), search.getLimit());

        List<UserMasterSonVO> userMasterSonVOList =  userMasterSonRefDao.selectMasterSonUserList(search);

        return new PageInfo<>(userMasterSonVOList);
    }

    @Override
    public PageInfo<UserMasterSonVO> companySonUserList(UserMasterSonSearch search){

        com.github.pagehelper.PageHelper.startPage(search.getPage(), search.getLimit());

        List<UserMasterSonVO> userMasterSonVOList = sysUserDao.selectCompanySonUserList(search);

        return new PageInfo<>(userMasterSonVOList);
    }

    @Override
    public void setUpMSType(UserMSTypeSet userMSTypeSet,LoginUser loginUser){

        // 获取账号企业
        Integer companyId = null;
        List<Integer> userIdList = userMSTypeSet.getUserIdList();
        SysUser sysUser = sysUserService.get(userIdList.get(0));
        if (null != sysUser) {
            companyId = sysUser.getCompanyId().intValue();
        }
        if (null == companyId) {
            throw new BizException("用户企业为空");
        }


        // 设置主账号
        if (UserMSTypeSet.msTypeEnum.master.name().equals(userMSTypeSet.getMsType())) {

            // 校验账号
            StringBuffer userCheckEx = checkMasterSonType(userIdList,0, companyId);

            if (null != userCheckEx) {

                userCheckEx.append("主子类型非普通账号不能设置为主账号");
                throw new BizException(userCheckEx.toString());
            }

            sysUserDao.updateMasterSonType(userIdList,2);
        }

        // 设置子账号
        if (UserMSTypeSet.msTypeEnum.son.name().equals(userMSTypeSet.getMsType())) {
            ...
            if (null != userCheckEx) {
                ...
                userCheckEx.append("主子类型非普通账号不能设置为子账号");
                throw new BizException(userCheckEx.toString());
            }

            sysUserDao.updateMasterSonType(userIdList,1);
        }

        // 取消主账号|子账号
        if (UserMSTypeSet.msTypeEnum.ordinary.name().equals(userMSTypeSet.getMsType())) {
            ...
            for (Integer userId : userIdList) {
                ...
                if (2 == sysUser.getMasterSonType()) {
                    ...
                    // 取消主账号
                    sysUserDao.updateMasterSonType(userIdList,0);
        ...
        if (UserMSTypeSet.msTypeEnum.addSon.name().equals(userMSTypeSet.getMsType())) {
            ...
            // 校验账号
            StringBuffer userCheckEx = checkMasterSonType(userIdList,1, companyId);

            if (null != userCheckEx) {

                userCheckEx.append("主子类型非子账号不能关联子账号");
                throw new BizException(userCheckEx.toString());
            }

            List<UserMasterSonRef> userMasterSonRefList = new ArrayList<>();
            UserMasterSonRef userMasterSonRef = null;

            for (Integer userId : userIdList) {

                userMasterSonRef = new UserMasterSonRef();

                userMasterSonRef.setMasterId(userMSTypeSet.getMasterUserId());
                userMasterSonRef.setSonId(userId);
                userMasterSonRef.setCompanyId(companyId);
                userMasterSonRef.setCreator(loginUser.getLoginName());
                userMasterSonRef.setGmtCreate(new Date());
                userMasterSonRef.setModifier(loginUser.getLoginName());
                userMasterSonRef.setGmtModified(new Date());
                userMasterSonRef.setIsDeleted(0);
                userMasterSonRef.setSysCode(UUID.randomUUID().toString().replaceAll("-",""));

                userMasterSonRefList.add(userMasterSonRef);
            }

            // 批量新增
            userMasterSonRefDao.bacthInsert(userMasterSonRefList);
        }


        // 移除子账号
        if (UserMSTypeSet.msTypeEnum.removeSon.name().equals(userMSTypeSet.getMsType())) {
            ...
            if (null != userCheckEx) {
                ...
                userCheckEx.append("主子类型非子账号不能移除子账号");
                throw new BizException(userCheckEx.toString());
            }
    ...
    private StringBuffer checkMasterSonType(List<Integer> userIdList,Integer masterSonType,Integer companyId){

        StringBuffer userEx = null;
        Integer count = sysUserDao.selectUserMsTypeCount(userIdList,companyId,masterSonType);
        if (count.intValue() != userIdList.size()) {

            userEx = new StringBuffer("[");
            SysUser sysUserCheck = null;
            for (Integer userId : userIdList) {

                sysUserCheck = sysUserService.get(userId);
                if (null != sysUserCheck ) {

                    if (sysUserCheck.getMasterSonType().intValue() != masterSonType ||sysUserCheck.getCompanyId().intValue() != companyId) {

                        if (userEx.length() < 2) {
                            userEx.append(sysUserCheck.getNickName());
                        } else {
                            userEx.append("," + sysUserCheck.getNickName());
                        }
                    }
