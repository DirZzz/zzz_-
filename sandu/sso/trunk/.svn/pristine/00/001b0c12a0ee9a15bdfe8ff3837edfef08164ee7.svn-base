...
public abstract class BaseUserManageServiceImpl {
     ...
     * @return
     */
    public UserBatchExcelVO addUser(UserAdd userAdd, Long userId){

        // 返回信息
        UserBatchExcelVO userBatchExcelVO = new UserBatchExcelVO();
        ...
        String passWord = StringUtils.isNotBlank(userAdd.getPassword()) ? userAdd.getPassword() : "123456";
        // 判断是否随机生成密码
        if (null != userAdd.getIsRandom() && userAdd.getIsRandom() > 0) {
            passWord = SysUserServiceImpl.getStringRandom(6);
        }
        // 初始密码
        sysUser.setInitialPassword(passWord);
        // 密码加密
        sysUser.setPassword(new Md5().getMd5Password(passWord));
        ...
        saveNotActiveVaildTime(addUserId,userAdd.getPriceId());

        //开通度币账号
        openPayAccount(sysUser.getId(), sysUser.getMobile(), 0d);

        // 赋值返回信息
        userBatchExcelVO.setUserId(addUserId);
        userBatchExcelVO.setNickName(nickName);
        userBatchExcelVO.setPassWord(passWord);

        return userBatchExcelVO;
    }
    ...
    public void editUser(UserEdit userEdit, Long userId){
        ...
        if (update > 0 && Objects.nonNull(userEdit.getServicesId())){
            ...
            Integer useType = syncAccountInfo(editUserInfo, userEdit.getServicesId());
             ...
             * 没有登录过的账号先保存用户时长记录
             */
            if (Objects.isNull(editUserInfo.getFailureTime())){
                saveNotActiveVaildTime(editUserInfo.getId(),userEdit.getPriceId());
            }
        }
    ...
    private void activatePackageTime(UserEdit userEdit, SysUser changeUserInfo, ServicesAccountRel account) {
        ...
        if (!CollectionUtils.isEmpty(servicesPrices)){
            servicesPrices.stream().filter(price -> Objects.equals(0,price.getIsDeleted())).findFirst().ifPresent(servicesPrice -> {
                Date now = new Date();
    ...
    private Integer  syncAccountInfo(SysUser editUserInfo, Long servicesId) {
...
//        }
    ...
    private void saveSystemField(SysUser sysUser, Long userId, Long servicesId) {
        ...
        //是否需要更新密码标识
        sysUser.setPasswordUpdateFlag(PASSWORD_NEED_UPDATE);
    ...
    protected PageInfo<UserManageDTO> selectUserList(UserManageSearch search){
        ...
        if (StringUtils.isNotBlank(search.getServicesName())){
            ...
            return sysUserService.findAllUserList(search);
        }
    }
