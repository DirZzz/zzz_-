...
import static com.sandu.common.exception.ExceptionCodes.CODE_10010021;
...
abstract class BaseUserLoginServiceImpl {
    ...
    public Object login(UserLogin userLogin) {
        ...
        //检查账号是否被冻结
        this.userAccountFreezeCheck(userInfoBO);
        ...
        userInfoBO.setPcModelVersion(userLogin.getPcModelVersion());
    ...
    private void userAccountFreezeCheck(UserInfoBO userInfoBO) {
        Date now = new Date();
        SysUser user = sysUserDao.selectByPrimaryKey(userInfoBO.getId().intValue());
        if (user.getAccountFreezeTime() != null && user.getAccountFreezeTime().before(now)) {
            if (user.getAccountEnableTime() == null || user.getAccountEnableTime().after(now)) {
                throw new BizException(CODE_10010021, "账号已被冻结，请及时续费");
            }
        }
    }
    ...
    public List<CompanyInfoBO> checkCompanyUserInfo(UserInfoBO userInfoBO, BasePlatform platform, Long currentLoginCompanyId) {
                ...
                if (companyId == null) {
                    ...
                    if (BasePlatform.PLATFORM_CODE_MOBILE_2B.equals(platform.getPlatformCode())
                        ...
                        if (null == userInfoBO.getBusinessAdministrationId() ||
                            ...
                            if (cid < 1) {
                                throw new BizException(CODE_10010021, "创建经销商企业失败!");
                            }
                        }
                    }
                } else {
        ...
        // 非户型绘制登录检查平台权限
        if (PlatFormConstant.PC_HOUSE_DRAW_VALUE != platform.getId().intValue()) {
    ...
    private int biuldVirtualCompany(UserInfoBO userInfoBO, BasePlatform platform) {
        ...
        Long factoryId = Optional.ofNullable(userInfoBO.getCompanyId()).orElseThrow(() -> new BizException(ExceptionCodes.CODE_10010021, "创建经销商企业失败!"));
        BaseCompany baseCompany = baseCompanyService.queryById(factoryId);
        Optional.ofNullable(baseCompany).orElseThrow(() -> new BizException(ExceptionCodes.CODE_10010021, "创建经销商企业失败,未找到对应的厂商!"));
        ...
        b.setIsVirtual(1);
        b.setIsDeleted(0);
        b.setPid(baseCompany.getId());
        b.setProductVisibilityRange(baseCompany.getProductVisibilityRange());
        b.setCompanyIndustrys(baseCompany.getCompanyIndustrys());
        b.setCategoryIds(baseCompany.getCategoryIds());
        //插入相对应的品牌
        List<BaseBrand> brandList =  baseBrandService.findCompanyBrandList(baseCompany.getId());
        if (!CollectionUtils.isEmpty(brandList)){
            //String brandIds = brandList.stream().filter(Objects::nonNull).map(item -> item.toString()).reduce(new StringBuilder(), (sb, s) -> sb.append(s).append(','), StringBuilder::append).toString();
            String brandIds = brandList.stream().filter(Objects::nonNull).map(item -> item.getId().toString()).collect(Collectors.joining(","));
            b.setBrandId(brandIds);
        }
...
//        logger.info("调用商家后台过滤过期角色");
        ...
        if (roleList != null && roleList.size() > 0) {
            ...
            for (SysRole r : roleList) {
                ...
                // PC端登录需设置户型绘制的角色
                if (null != platformCode && null != r.getPlatformId() && platformCode.equals(BasePlatform.PLATFORM_CODE_PC_2B) && r.getPlatformId().intValue() == PlatFormConstant.PC_HOUSE_DRAW_VALUE) {
    ...
    public UserInfoBO checkUserAccount(UserLogin userLogin) {
        ...
        if (null == list || list.size() == 0) {
            list = checkSecretFreeCompany(userLogin);
        }
        ...
        if (userLogin.getPlatformType() == SysUser.PLATFORM_TYPE_B) {
            if (list != null && list.size() > 1) {
                ...
                //获取最近一个登录该平台的账号
                List<Long> userIds = list.stream().map(UserInfoBO::getId).collect(Collectors.toList());
                List<SysUserLoginLog> sysUserLoginLog = sysUserLoginLogService.findLoginLastAccount(userIds,userLogin.getPlatformId());
                System.out.println(sysUserLoginLog);
                if (!CollectionUtils.isEmpty(sysUserLoginLog)){
                    if (sysUserLoginLog.get(0) != null){
                        List<UserInfoBO> userList = list.stream()
                                .filter(item -> Objects.equals(sysUserLoginLog.get(0).getUserId(), item.getId()))
                                .collect(Collectors.toList());
    ...
    private List<UserInfoBO> checkSecretFreeCompany(UserLogin userLogin){

        String secreFreeCompany = "";
        List<UserInfoBO> list = null;

        // 获取定制企业免密登录
        SysDictionary sysDictionary = new SysDictionary();
        sysDictionary.setType("secretFreeCompany");
        ...
        if (null != dictionaryList && dictionaryList.size() > 0) {
            ...
            secreFreeCompany = dictionaryList.stream().map(o -> o.getValue().toString()).collect(Collectors.joining(","));
        }
        logger.info("定制企业免密登录=" + secreFreeCompany);
        logger.info("用户参数userLogin=" + new Gson().toJson(userLogin));


        // 获取当前用户对于定制企业
        BaseCompany company = null;
        if (StringUtils.isNotBlank(userLogin.getCompanyCode())) {
            company = baseCompanyService.getCompanyByCode(userLogin.getCompanyCode());
        } else {
            company = baseCompanyService.getByNickName(userLogin.getAccount());
        }

        if(null != company) {

            // 校验该定制企业是否支持免密登录
            if (StringUtils.isNotBlank(secreFreeCompany) && secreFreeCompany.contains(company.getId() + "")) {
