...
public class PcUserLoginServiceImpl extends BaseUserLoginServiceImpl implements UserLoginService {
    ...
    @Value("${pc.model.version}")
    private String pcModelVersion;
    ...
    public void createUserJwtToken(UserInfoBO userInfoBO) {
        ...
        String modelType = "old";
        if (StringUtils.isNotBlank(pcModelVersion) && StringUtils.isNotBlank(userInfoBO.getPcModelVersion())
                && pcModelVersion.equals(userInfoBO.getPcModelVersion())) {
            modelType = "new";
        }
