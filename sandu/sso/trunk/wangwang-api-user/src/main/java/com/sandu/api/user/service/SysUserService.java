...
public interface SysUserService {
    ...
    PageInfo<UserManageDTO> findAllUserList(UserManageSearch search);

	List<Map<String,Object>> findUserPackageInfoByUserIds(List<Long> userIds);

	void updateFreezeState(SysUser sysUser);

	boolean activePackageUserTime(Integer servicesId,Date failuerTime,Integer userId);

	boolean syncAccountInfo(SysUser editUserInfo, Long servicesId);

	void batchModifyPassword(boolean random, String password, String loginName, String userIds);

	boolean batchModifyPackage(List<Integer> userIds, Integer servicesId);

	List<SysUser> listByIds(List<Integer> collect);

	void saveOperatorLog(Long userId,Integer priceId);
	 ...
	 * @return
	 */
	Integer getMasterUserId(Integer userId);
}
