...
public class CompanyShopMetaDataStorage {
    ...
    private final Map<Integer, Integer> companyId2ShopId = new HashMap<>(128);
    ...
    public Map<Integer, Integer> companyId2CompanyBrandShopId() {
        if (!CollectionUtils.isEmpty(companyId2ShopId)) {
            return this.companyId2ShopId;
        }
        List<CompanyShopPo> list = metaDataService.companyId2CompanyBrandShopId();
        Map<Integer, Integer> collect = list.stream().collect(Collectors.toMap(CompanyShopPo::getCompanyId, CompanyShopPo::getId));
        this.companyId2ShopId.putAll(collect);
        return this.companyId2ShopId;
    }
}
