...
public class ProductUsageCountMetaDataStorage {
     ...
     * 全量同步之后缓存用户使用产品数量
     */
    public void cacheUserUsageCount() {
        //产品使用次数元数据
        List<ProductUsagePo> productUsagePoList;
        try {
            //获取数据
            ...
            throw new NullPointerException(CLASS_LOG_PREFIX + "获取产品用户使用次数元数据失败,List<ProductTexturePo> is null.MetaDataException:" + e);
        }

        //临时对象
        Map<Integer, Integer> tempUserUsageCountMap = new HashMap<>();
        ...
        if (null != productUsagePoList && 0 != productUsagePoList.size()) {
            productUsagePoList.forEach(productUsagePo -> {
                //用户ID
                int userId = productUsagePo.getUserId();
                //add by xiaoxc-20190115 (排序判断用户是否存在已使用产品)
                if (tempUserUsageCountMap.containsKey(userId)) {
                    tempUserUsageCountMap.put(userId, tempUserUsageCountMap.get(userId) + 1);
                } else {
                    tempUserUsageCountMap.put(userId, 1);
                }
            });
        }

        //数据转换
        Map<String, String> userUsageCountJsonMap = new HashMap<>(tempUserUsageCountMap.size());
        tempUserUsageCountMap.forEach((k, v) -> userUsageCountJsonMap.put(k + "", JsonUtil.toJson(v)));
        log.info(CLASS_LOG_PREFIX + "格式化产品使用次数Json元数据完成....");

        //装载缓存
        redisService.del(RedisConstant.USER_USAGE_PRODUCT_COUNT_DATA);
        redisService.addMapCompatible(RedisConstant.USER_USAGE_PRODUCT_COUNT_DATA, userUsageCountJsonMap);
        log.info(CLASS_LOG_PREFIX + "产品使用次数元数据装载缓存完成....");
     ...
     * @return
     */
    public Integer getUserUsageProductCountMetaData(Integer userId) {

        if (null == userId || 0 >= userId) {
            return null;
        }

        String mapStr = getMap(RedisConstant.USER_USAGE_PRODUCT_COUNT_DATA, userId + "");
        if (StringUtils.isEmpty(mapStr)) {
            return null;
        }
