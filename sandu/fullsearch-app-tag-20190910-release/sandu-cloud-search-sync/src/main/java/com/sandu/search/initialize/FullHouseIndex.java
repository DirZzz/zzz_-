...
public class FullHouseIndex {
    ...
    public int indexFullHousePlanData(List<RecommendationPlanPo> recommendationPlanList, String type) {
        ...
        //方案ID：相关区域报价信息
        Map<Integer, List<PlanQuotedPricePO>> id2Prices = fullHouseDesignPlanIndexService.queryFullHouseQuotedPriceByIds(recommendationPlanList.stream().map(RecommendationPlanPo::getFullHouseId).collect(Collectors.toList()))
        ...
        for (int i = 0; i < recommendationPlanList.size(); i++) {
            ...
            // 方案报价信息(XZW-520)
            recommendationPlanIndexMappingData.setPlanQuotedPriceList(id2Prices.get(recommendationPlanPo.getFullHouseId()));

            log.info("testst----recommendationPlanIndexMappingData---{}", recommendationPlanIndexMappingData);
            log.info("testst----recommendationPlanPo---{}", recommendationPlanPo);
            //创建索引对象 //全屋用uuid
            IndexRequestDTO indexRequestDTO = new IndexRequestDTO(
                    elasticSearchConfig.getRecommendedPlanDataIndexName(),
