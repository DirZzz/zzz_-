...
public class DesignPlanPlatformMetaDataStorage {
        ...
        if (null != designPlanPlatformRelPoList && 0 != designPlanPlatformRelPoList.size()) {
            for(DesignPlanPlatformRelPo designPlanPlatformRelPo : designPlanPlatformRelPoList) {
                ...
                if (0 < designPlanPlatformRelPo.getPlanId() && !StringUtils.isEmpty(designPlanPlatformRelPo.getPlatformCode())) {
                    ...
                    getDesignPlanPlatfromDate(designPlanPlatformRelPo, designPlanPlatformData);
                    //装入Map
    ...
    public DesignPlanPlatformData queryPlanPlatformByPlanId(Integer planId, String logicType, Integer planType) {
        ...
        //log.info(CLASS_LOG_PREFIX + "平台信息，planPlatformRelStr:{}", planPlatformRelStr);
        if (StringUtils.isEmpty(planPlatformRelStr)) {
            log.error(CLASS_LOG_PREFIX + "平台信息为空，planId:{},logicType:{}", planId + "_" + planType, logicType);
            return this.selectPlanPlatfromData(planId, planType);
        }
     ...
     * @return
     */
    private DesignPlanPlatformData selectPlanPlatfromData(Integer planId, Integer planType) {
        if (PlatformConstant.PLANFORM_DESING_PLAN_TYPE_ONEKEY == planType) {
            planType = PlatformConstant.PLANFORM_DESING_PLAN_TYPE_OPEN;
        }
        List<Integer> recommendedPlanIdList = new ArrayList<>(Arrays.asList(planId));
        //方案平台元数据
        List<DesignPlanPlatformRelPo> designPlanPlatformRelPoList = null;
        try {
            if (null != recommendedPlanIdList && 0 < recommendedPlanIdList.size()) {
                designPlanPlatformRelPoList = metaDataService.queryPlatformPlanByIdsMetaData(recommendedPlanIdList, planType);
            }
        } catch (MetaDataException e) {
            log.error(CLASS_LOG_PREFIX + "获取方案平台元数据失败: MetaDataException:{}", e);
            throw new NullPointerException(CLASS_LOG_PREFIX + "designPlanPlatformRelPoList is null.MetaDataException:" + e);
        }
        //方案平台对象
        DesignPlanPlatformData designPlanPlatformData = new DesignPlanPlatformData();
        if (null != designPlanPlatformRelPoList && 0 != designPlanPlatformRelPoList.size()) {
        ...
        return designPlanPlatformData;
    }

    /**
     * 获取不同端的平台信息
     * @param designPlanPlatformRelPo
     * @param designPlanPlatformData
     * @return
     */
    private DesignPlanPlatformData getDesignPlanPlatfromDate(DesignPlanPlatformRelPo designPlanPlatformRelPo, DesignPlanPlatformData designPlanPlatformData) {
