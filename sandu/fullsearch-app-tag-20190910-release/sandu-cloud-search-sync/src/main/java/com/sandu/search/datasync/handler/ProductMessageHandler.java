...
public class ProductMessageHandler {
    ...
    public boolean deleteByIds(List<Integer> idList) throws ElasticSearchException {
        ...
        if (null == idList || 0 >= idList.size()) {
            log.info(CLASS_LOG_PREFIX + "同步删除产品失败，同步对象为空。");
            return false;
        }
        String indexName = StringUtils.isEmpty(elasticSearchConfig.getReIndexProductDataIndexName())? IndexConstant.RECOMMENDATION_PLAN_ALIASES:elasticSearchConfig.getRecommendedPlanDataIndexName();
        for (Integer id : idList) {
            ...
            boolean falg = elasticSearchService.delete(deleteRequestDTO);
            if (!falg) {
                log.error(CLASS_LOG_PREFIX + "同步删除产品失败,id:{}", id);
            }
        }
