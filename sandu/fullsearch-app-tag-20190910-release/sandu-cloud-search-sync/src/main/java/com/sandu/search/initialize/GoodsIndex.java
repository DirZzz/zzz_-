...
import com.sandu.search.service.metadata.impl.MetaDataServiceImpl;
...
public class GoodsIndex {
    ...
    public GoodsIndex(GoodsIndexService goodsIndexService, ElasticSearchConfig elasticSearchConfig, ProductPlatformMetaDataStorage productPlatformMetaDataStorage, ProductCategoryRelMetaDataStorage productCategoryRelMetaDataStorage,
                      ...
                      BaseProductDataStorage baseProductDataStorage, MetaDataService metaDataService) {
    ...
    public int indexGoodsData(List<BaseGoodsSpuPo> goodsPoList) {
        ...
        for (BaseGoodsSpuPo baseGoodsSpuPo : goodsPoList) {
            ...
            goodsIndexMappingData.setSpuSort(baseGoodsSpuPo.getSort());
            ...
            //处理品牌馆ID
            Map<Integer, Integer> id2ShopId = companyShopMetaDataStorage.companyId2CompanyBrandShopId();
//            List<CompanyShopPo> shops = companyShopMetaDataStorage.getCompanyShopPoByCompanyId(baseGoodsSpuPo.getCompanyId());
//            if (!CollectionUtils.isEmpty(shops)) {
//                GoodsIndexMappingData finalGoodsIndexMappingData = goodsIndexMappingData;
//                shops.stream().filter(it -> it.getBusinessType().equals(Integer.valueOf(1))).findFirst().ifPresent(it -> finalGoodsIndexMappingData.setGoodsCompanyBrandShopId(it.getId()));
//            }
            Integer shopId = id2ShopId.get(baseGoodsSpuPo.getCompanyId());
            goodsIndexMappingData.setGoodsCompanyBrandShopId(shopId == null ? 0 : shopId);
            ...
            //处理商品中每一个单元的sku数据
            List<BaseGoodsSkuPo> baseGoodsSkuPoList = new LinkedList<>();
            ...
            if (null != baseProductDataPoList && baseProductDataPoList.size() > 0) {
                ...
                BaseGoodsSkuPo defaultGoods = goodsSkuMetaDataStorage.fetchDefaultGoods(baseGoodsSkuPoList);
                goodsIndexMappingData.setGoodsDefaultPrice(defaultGoods.getPrice());
                goodsIndexMappingData.setGoodsDefaultProductId(defaultGoods.getProductId());
