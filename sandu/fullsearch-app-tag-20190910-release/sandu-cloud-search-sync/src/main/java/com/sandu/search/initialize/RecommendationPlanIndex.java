...
public class RecommendationPlanIndex {
    ...
    public int indexRecommendationPlanData(List<RecommendationPlanPo> recommendationPlanList, String updateType) {
        ...
        //xzw-520 方案ID：相关区域报价信息
        Map<Integer, List<PlanQuotedPricePO>> id2Prices = designPlanIndexService.queryDesignPlanQuotedPriceByIds(recommendationPlanList.stream().map(RecommendationPlanPo::getId).collect(Collectors.toList()))
                .stream().distinct().collect(Collectors.groupingBy(PlanQuotedPricePO::getPlanId));
        ...
        for (int i = 0; i < recommendationPlanList.size(); i++) {
            ...
            //方案企业报价（XZW-520）
            recommendationPlanIndexMappingData.setPlanQuotedPriceList(id2Prices.get(recommendationPlanPo.getId()));
        ...
        log.info(CLASS_LOG_PREFIX + "解析推荐方案数据完成，开始索引数据...indexName:" + elasticSearchConfig.getRecommendedPlanDataIndexName());
        //索引数据
        BulkStatistics bulkStatistics = null;
        try {
            bulkStatistics = elasticSearchService.bulk(bulkIndexList, null);
    ...
    public void getRecommendedPlanPlatformInfoById(RecommendationPlanIndexMappingData recommendationPlanIndexMappingData, String logicType) {
        ...
        //log.info(CLASS_LOG_PREFIX + "平台信息，designPlanPlatformData:{}", JsonUtil.toJson(designPlanPlatformData));
