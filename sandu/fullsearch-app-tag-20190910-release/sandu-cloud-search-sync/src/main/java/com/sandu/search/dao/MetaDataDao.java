...
public interface MetaDataDao {
    ...
    List<BaseProductDataPo> queryBaseProductData(@Param("start") int start, @Param("limit") int limit);
     ...
     * @return
     */
    List<UsedProductsPo> queryLastMonthUsedProductData(@Param("startTime") String startTime, @Param("endTime") String endTime);

    /**
     * 获取平台组合ToB端元数据
     * @return
     */
    List<GroupPlatformRelPo> queryToBPlatformGroupMetaData();

    /**
     * 获取平台组合ToC端元数据
     * @return
     */
    List<GroupPlatformRelPo> queryToCPlatformGroupMetaData();

    List<Integer> selectLivingIds(@Param("start") int start, @Param("limit")int limit);
