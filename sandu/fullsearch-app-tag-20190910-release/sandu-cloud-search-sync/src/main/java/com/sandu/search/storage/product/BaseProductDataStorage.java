...
public class BaseProductDataStorage {
    ...
    public void updateData() {
        ...
        //临时对象
        Map<Integer, List<BaseProductDataPo>> tempBaseProductDataListMap = new HashMap<>(10 * limit);
        boolean isContinueHandler = true;
        while (isContinueHandler) {
            //基础产品元数据
            List<BaseProductDataPo> baseProductDataList;
            try {
                baseProductDataList = metaDataService.queryBaseProductData(start, limit);
            } catch (MetaDataException e) {
                log.error(CLASS_LOG_PREFIX + "获取基础产品元数据失败: MetaDataException:{}", e);
                throw new NullPointerException(CLASS_LOG_PREFIX + "获取基础产品元数据失败,List<ProductAttributePo> is null.MetaDataException:" + e);
            }
            ...
            //无数据中断操作
            if (null == baseProductDataList || 0 == baseProductDataList.size()) {
                log.info(CLASS_LOG_PREFIX + "查询产品数据为空：start:{},limit:{}.", start, limit);
                break;
            }
            //数据不足指定数据量表示已查询出最后一条数据,终止循环
            if (baseProductDataList.size() < IndexInfoQueryConfig.DEFAULT_QUERY_PRODUCTPOINFO_LIMIT) {
    ...
    public  List<BaseProductDataPo> getBaseProductListBySpuId(Integer id) {
        ...
        if (CollectionUtils.isEmpty(baseProductDataPoList)) {
            baseProductDataPoList = this.queryBaseProductDataListById(id);
            log.warn("从缓存中获取商品对应的产品数据失败，改为从db获取产品数据，spuId:{}", id);
        }
