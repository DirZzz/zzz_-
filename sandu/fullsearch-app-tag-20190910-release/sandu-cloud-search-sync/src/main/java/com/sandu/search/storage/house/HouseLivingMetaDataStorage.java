...
public class HouseLivingMetaDataStorage {
    ...
    public List<Integer> listLivingId(int start, int limit) throws LivingIndexException{
        if (limit == 0){
            limit = IndexInfoQueryConfig.DEFAULT_QUERY_LIVINGPOINFO_LIMIT;
        }

        log.info(CLASS_LOG_PREFIX + "正在查询小区id信息第{}-{}条.", start, (start + limit));
        List<Integer> livingIds;
        try {
            livingIds = metaDataService.selectLivingIds(start,limit);
        }catch (Exception e){
            log.error(CLASS_LOG_PREFIX + "获取小区id数据第{}-{}条失败,Exception:"+e, start, (start + limit));
            throw new LivingIndexException(CLASS_LOG_PREFIX + "获取小区数据第" + start + "-" + (start + limit) + "条失败,Exception:" + e);
        }
        return livingIds;
    }
}
