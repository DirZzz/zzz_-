...
public class ProductIndex {
    ...
    public void syncProductInfoData() {
        ...
        while (isContinueHandler) {
            ...
            if (start == 0) {
                //同步产品之前先清空产品属性code缓存
                redisService.del(RedisConstant.PRODUCT_ATTRIBUT_KEY_LIST);
            }

            //全量同步之后缓存用户使用产品数量
            if (!isContinueHandler) {
                productUsageCountMetaDataStorage.cacheUserUsageCount();
            }
            //递增start下标
            start = start + limit;
    ...
    public int indexProdcutData(List<ProductPo> productPoList) {
        ...
        /********************************** 处理产品分类信息数据 *********************************/
        Set attributekeySet = new HashSet(50);
        for (ProductPo productPo : productPoList) {
            //索引产品的属性集合
            String attributekeys = productPo.getProductAttributeKeys();
            if (!StringUtils.isEmpty(attributekeys)) {
                attributekeySet.addAll(Arrays.asList(attributekeys.split(",")));
            }
            ...
            productIndexMappingData.setProModelNumber(productPo.getProductModelNumber());
            ...
            //上个月的产品使用次数数据 add by xiaoxc-20190110
            Integer lastMonthProductUsageCount = usedProductsMetaDataStorage.getUsedProductCountMetaDataMap(productIndexMappingData.getId());
            if (null != lastMonthProductUsageCount) {
                productIndexMappingData.setLastMonthProductUsageCount(lastMonthProductUsageCount);
            }
        ...
        //缓存索引产品的属性CODE数据
        List<String> productAttrList = redisService.getList(RedisConstant.PRODUCT_ATTRIBUT_KEY_LIST);
        if (null != productAttrList && productAttrList.size() > 0) {
            attributekeySet.addAll(productAttrList);
        }
        if (null != attributekeySet && attributekeySet.size() > 0) {
            //先清空在加入缓存
            redisService.del(RedisConstant.PRODUCT_ATTRIBUT_KEY_LIST);
            redisService.addList(RedisConstant.PRODUCT_ATTRIBUT_KEY_LIST, new ArrayList<>(attributekeySet));
        }
