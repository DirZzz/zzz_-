...
public class HouseIndexServiceImpl implements HouseIndexService {
    ...
    @Override
    public List<HouseLivingPo> listHouseByLivingIds(List<Integer> livingIds) {
        if (CollectionUtils.isEmpty(livingIds)) return Collections.EMPTY_LIST;
        return houseIndexDao.selectHouseByLivingIds(livingIds);
    }

    @Override
    public List<Integer> queryHouseChangeBeforeOneMin() {
        return houseIndexDao.selectHouseChangeBeforeOneMin();
    }
}
