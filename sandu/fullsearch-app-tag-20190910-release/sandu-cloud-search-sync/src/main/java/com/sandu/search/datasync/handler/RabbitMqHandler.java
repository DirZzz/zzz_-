...
public class RabbitMqHandler {
    ...
    @Autowired
    public RabbitMqHandler(GroupProductMessageHandler groupProductMessageHandler,
    ...
    public void consumerMessage(String message) {
        ...
        switch (moduleType) {
            ...
            case QueueMessage.SHOPPLAN_MODULE:
                //操作类型
                if (QueueMessage.UPDATE_ACTION == actionType || QueueMessage.DELETE_ACTION == actionType) {
                    //更新店铺平台
                    messageConsumerStatus = recommendationMessageHandler.updatePlanShopInfo(JsonUtil.fromJson(queueMessage.getObject().toString(), new TypeToken<List<CompanyShopPlanPo>>() {
                    }.getType()));
                } else if (QueueMessage.DELETE_ACTION == actionType) {
                    //删除店铺
            ...
            case QueueMessage.GROUP_PRODUCT_MODULE:
                ...
                if (QueueMessage.UPDATE_ACTION == actionType) {
                    //更新
                    messageConsumerStatus = groupProductMessageHandler.addAndUpdate(JsonUtil.fromJson(queueMessage.getObject().toString(), new TypeToken<List<RecommendationPlanIndexMappingData>>() {
                    }.getType()));
                } else if (QueueMessage.DELETE_ACTION == actionType) {
                    //删除
                    try {
                        messageConsumerStatus = groupProductMessageHandler.delete(JsonUtil.fromJson(queueMessage.getObject().toString(), new TypeToken<List<ProductIndexMappingData>>() {
                        }.getType()));
                    } catch (ElasticSearchException e) {
                        log.error(CLASS_LOG_PREFIX + "删除组合产品失败，object：{},e:{}", JsonUtil.toJson(queueMessage.getObject()), e);
                    }
                }
                break;
            default:
