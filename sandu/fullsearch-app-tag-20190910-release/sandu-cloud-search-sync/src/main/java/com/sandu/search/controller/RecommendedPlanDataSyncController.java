...
import com.sandu.search.entity.SyncRecommendedPlanVo;
...
public class RecommendedPlanDataSyncController {
    ...
    @Autowired
    public RecommendedPlanDataSyncController(StorageComponent storageComponent, RecommendationMessageHandler recommendationMessageHandler, DesignPlanIndexService designPlanIndexService) {
    ...
    @PostMapping("/sync/{id}/{likeNum}/{collectNum}/{commentsNum}")
    public String syncLikeAndCollectAndCommentOfNumInfo(@PathVariable String id, Integer likeNum, Integer collectNum, Integer commentsNum) {
        UpdateResponse response = designPlanIndexService.updatePlanLikeAndCollectAndCommentOfNumInfo(id,likeNum,collectNum,commentsNum);
        log.info(CLASS_LOG_PREFIX + "远程调用更新完成！");
        return "success";
    }
