...
import static com.sandu.search.common.constant.QueryConditionField.QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST;
import static com.sandu.search.common.constant.QueryConditionField.QUERY_CONDITION_FIELD_PRODUCT_STYLE_IDS;
...
public class SXWProductSearchServiceImpl implements SXWProductSearchService {
    ...
    public SearchObjectResponse searchProduct(ProductSearchMatchVo productSearchMatchVo,
        ...
        if (productSearchMatchVo.getMatchQueryList() != null && productSearchMatchVo.getMatchQueryList().size() > 0) {
            matchQueryList.addAll(productSearchMatchVo.getMatchQueryList());
        }
        //非匹配条件List
        ...
        /********************** 1.上架状态和平台过滤 **********************/
        // 添加平台过滤
        QueryBuilder platformCodeQuery = QueryBuilders.matchQuery(QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST + "." + QueryConditionField.QUERY_CONDITION_FIELD_PRODUCT_PLATFORM_CODE_LIST, productSearchMatchVo.getPlatformCode());
        matchQueryList.add(QueryBuilders.nestedQuery(QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST, platformCodeQuery, ScoreMode.Max));

        // 通配符匹配
        if (productSearchMatchVo.getWildcardMatchSearchList() != null) {
            for (MultiWildcardQuery tmp : productSearchMatchVo.getWildcardMatchSearchList()) {
                if (tmp.getQuery() != null) {
                    matchQueryList.add(tmp.getQuery());
                }
            }
        }

        //风格搜索
        if (null != productSearchMatchVo.getStyleId() && productSearchMatchVo.getStyleId() > 0) {
            MatchQueryBuilder matchQueryBuilder = QueryBuilders.matchQuery(QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST + "." + QUERY_CONDITION_FIELD_PRODUCT_STYLE_IDS, productSearchMatchVo.getStyleId());
            matchQueryList.add(matchQueryBuilder);
        ...
        // 添加商品上架状态过滤
        matchQueryList.add(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_GOODS_IS_PUTAWAY, 1));

        // 添加平台上架状态过滤
        putawayStateBoolQueryBuilder.must(QueryBuilders.termQuery(QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST
                        + "." +
        ...
        matchQueryList.add(QueryBuilders.nestedQuery(QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST, putawayStateBoolQueryBuilder, ScoreMode.Max));
       ...
       /* //组合查询条件--产品品牌ID列表
                ...
                productFiveCategoryLongCodeList.forEach(productCategoryLongCode ->
                        categoryBoolQueryBuilder.must(
                                QueryBuilders.termQuery(
                                        QUERY_CONDITION_FIELD_GOODS_SKU_PO_LIST
                                                + "." +
        ...
        //组合查询条件--单值匹配多字段--List中第一个字段优先级最高 match 条件
        setMatchQueryCondition(productSearchMatchVo, matchQueryList);
        // 排序字段
        // 如果查询的产品名称为空，根据修改时间排序，否则根据相关性排序
        List<SortOrderObject> sortOrderObjectList = new ArrayList<>();
        if ("goodsDefaultPrice".equals(productSearchMatchVo.getOrderField())) {
            sortOrderObjectList.add(new SortOrderObject("goodsDefaultPrice", productSearchMatchVo.getSortOrder(), SortMode.MAX, SortOrderObject.DEFAULT_SORT));
        }
        if (productSearchMatchVo.getKeySearch()) {
            sortOrderObjectList.add(new SortOrderObject(SortBuilders.scoreSort(), SortOrderObject.TRACK_SCORES));
        }
        if (productSearchMatchVo.getCompanyId() == 0) {
            sortOrderObjectList.add(new SortOrderObject("spuSort", SortOrder.DESC, SortMode.MAX, SortOrderObject.DEFAULT_SORT));
        }
        if (StringUtils.isEmpty(productSearchMatchVo.getProductName())) {
            sortOrderObjectList.add(new SortOrderObject("id", productSearchMatchVo.getSortOrder(), SortMode.MAX, SortOrderObject.DEFAULT_SORT));
        }
        ...
        try {
            searchObjectResponse = elasticSearchService.search(matchQueryList, noMatchQueryList, shouldMatchSearchList, aggregationBuilderList, sortOrderObjectList, pageVo.getStart() - 1, pageVo.getPageSize(), IndexConstant.SXW_PRODUCT_INFO);
    ...
    private void setMatchQueryCondition(ProductSearchMatchVo productSearchMatchVo, List<QueryBuilder> matchQueryList) {
