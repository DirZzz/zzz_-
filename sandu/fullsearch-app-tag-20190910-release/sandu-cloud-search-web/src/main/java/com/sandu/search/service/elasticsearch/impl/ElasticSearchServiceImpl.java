...
public class ElasticSearchServiceImpl implements ElasticSearchService {
    ...
    public SearchObjectResponse search(
        ...
        if (null != sortOrderObjectList && 0 < sortOrderObjectList.size()) {
            sortOrderObjectList.forEach(sortOrderObject -> {
                ...
                //分数强制排序
                if (SortOrderObject.TRACK_SCORES.equals(sortOrderObject.getSortType())) {
                    sourceBuilder.sort(sortOrderObject.getScoreSort());
                }
            });
        }
        ...
        try {
            ...
            log.error(CLASS_LOG_PREFIX + "Excption:{}", e);
            throw new ElasticSearchException(CLASS_LOG_PREFIX + "搜索数据异常excption:{}", e);
        }
            ...
            if (!StringUtils.isEmpty(searchHit.getSourceAsString())) {
                ...
                switch (index) {
                    ...
                    case IndexConstant.RECOMMENDATION_PLAN_ALIASES :
                        ...
                        break;
                    //组合产品
                    case IndexConstant.GROUP_PRODUCT_ALIASES :
                        clazz = GroupProductPo.class;
                        break;
                    default:
     ...
     * @return
     */
    public List<Map<String, Object>> searchScrollData(SearchSourceBuilder sourceBuilder,String index,Integer dataSize) throws ElasticSearchException {
        // 参数验证
        if(sourceBuilder == null) {
            log.error(CLASS_LOG_PREFIX + "查询条件为空!sourceBuilder:{}", JsonUtil.toJson(sourceBuilder));
            throw new ElasticSearchException(CLASS_LOG_PREFIX + "参数异常");
