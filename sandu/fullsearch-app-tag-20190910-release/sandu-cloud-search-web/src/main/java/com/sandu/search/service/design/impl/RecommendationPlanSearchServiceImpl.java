...
public class RecommendationPlanSearchServiceImpl implements RecommendationPlanSearchService {
    ...
    public SearchObjectResponse searchRecommendationPlan(RecommendationPlanSearchVo recommendationPlanSearchVo, PageVo pageVo) throws RecommendationPlanSearchException {
        ...
        if(recommendationPlanSearchVo.getPlatformCode().equals(PlatformConstant.PLATFORM_CODE_SELECT_DECORATION)){
            ...
            if(ids != null && ids.size() > 0){
                ...
                boolQueryBuilder.must(QueryBuilders.boolQuery().mustNot(QueryBuilders.idsQuery().addIds(idStr)));
    ...
    private BoolQueryBuilder searchFullHousePlan(RecommendationPlanSearchVo recommendationPlanSearchVo) {
        ...
        if (null != companyInfoBoolQueryBuilder) {
            searchFullHouseBoolQueryBuilder.must(companyInfoBoolQueryBuilder);
        }

        //风格
        Integer designStyleId = recommendationPlanSearchVo.getDesignStyleId();
        if (null != designStyleId && 0 < designStyleId) {
            searchFullHouseBoolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_DESIGN_STYLEID, designStyleId));
        }

        //方案类型(2:全屋方案)
        searchFullHouseBoolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLANA_TABLE_TYPE, DesignPlanType.FULLHOUSE_TABLE_TYPE));

        //数据未被删除
        searchFullHouseBoolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_STATUS, 0));
        ...
        if (null != boolQueryBuildersOfDecoratePrice) {
            searchFullHouseBoolQueryBuilder.must(boolQueryBuildersOfDecoratePrice);
        }
    ...
    private BoolQueryBuilder searchDragDecorateRecommendationPlan(RecommendationPlanSearchVo recommendationPlanSearchVo, List<SortOrderObject> sortOrderObjectList) {
        ...
        searchRecommendedPlanBooleQueryBuilder.must(boolQueryBuilder);
    ...
    private BoolQueryBuilder searchDecorateOrOpenOrPrototypeRecommendationPlan(RecommendationPlanSearchVo recommendationPlanSearchVo, List<SortOrderObject> sortOrderObjectList) {
        ...
        if (!PlatformConstant.PLATFORM_CODE_SELECT_DECORATION.equals(recommendationPlanSearchVo.getPlatformCode())){
            BoolQueryBuilder boolQueryBuildersOfPlatformCode = getBoolQueryBuildersOfPlatformCode(recommendationPlanSearchVo.getPlatformCode());
            if (null != boolQueryBuildersOfPlatformCode) {
                searchRecommendedPlanBooleQueryBuilder.must(boolQueryBuildersOfPlatformCode);
            }
        }
    ...
    private BoolQueryBuilder searchTestOrPublishypeRecommendationPlan(RecommendationPlanSearchVo recommendationPlanSearchVo, List<SortOrderObject> sortOrderObjectList) {
        ...
        Integer userId = recommendationPlanSearchVo.getUserId();
        if (null != userId && 0 < userId) {
            searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLATFORM_CREATE_USER_ID, userId));
        }
    ...
    private void searchRecommendedPlanCommonCondtion(RecommendationPlanSearchVo recommendationPlanSearchVo, BoolQueryBuilder searchRecommendedPlanBooleQueryBuilder, List<SortOrderObject> sortOrderObjectList) {
        ...
        if (!RecommendationPlanPo.FUNCTION_TYPE_DRAGDECORATE.equals(disPlayType)) {
            ...
            if (null != companyId && 0 < companyId) {
                BoolQueryBuilder companyInfoBoolQueryBuilder = getBoolQueryBuildersOfCompanyIdList(recommendationPlanSearchVo);
                if (null != companyInfoBoolQueryBuilder) {
                    searchRecommendedPlanBooleQueryBuilder.must(companyInfoBoolQueryBuilder);
                }
            }
            ...
            if (null != releaseStatus) {
                searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_RELEASE_STATUS, releaseStatus));
            }
        }
        ...
        if (null != houseType && 0 < houseType) {
            searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_HOUSE_TYPE, houseType));
        }

        //风格
        Integer designStyleId = recommendationPlanSearchVo.getDesignStyleId();
        if (null != designStyleId && 0 < designStyleId) {
            searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_DESIGN_STYLEID, designStyleId));
        }
        ...
        if (null != recommendationPlanType && 0 < recommendationPlanType) {
            searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_RECOMMENDATIONPLAN_TYPE, recommendationPlanType));
        }
        ...
        if (!StringUtils.isEmpty(shelfStatus) && (null == shopId || 0 == shopId)) {
            if (shelfStatus.contains(",")){
                ...
                searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termsQuery(
                        QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_SHELF_STATUS_LIST,
                        shelfStatusList));
            }else {
                searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_SHELF_STATUS_LIST, shelfStatus));
            }
        }
        ...
        if (null != spaceAreas && spaceAreas > 0) {
            ...
            if (RecommendationPlanPo.FUNCTION_TYPE_DRAGDECORATE.equals(displayType)
                ...
                searchRecommendedPlanBooleQueryBuilder.must(spaceAreasBool);
            } else {
                searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_SPACE_AREAS, spaceAreas));
            }
        }
        ...
        if (null != planSourceList && 0 < planSourceList.size()) {
            searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_SOURCE, planSourceList));
        }

        //数据未被删除
        searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_STATUS, 0));
        ...
        searchRecommendedPlanBooleQueryBuilder.must(groupPrimaryBool);
        ...
        BoolQueryBuilder boolQueryBuildersOfDecoratePrice = getBoolQueryBuildersOfDecoratePrice(decoratePriceType, decoratePriceRenge);
        if (null != boolQueryBuildersOfDecoratePrice) {
            searchRecommendedPlanBooleQueryBuilder.must(boolQueryBuildersOfDecoratePrice);
        }

        List<Integer> planIdList = recommendationPlanSearchVo.getPlanIdList();
        if (!Lists.isEmpty(planIdList)) {
            searchRecommendedPlanBooleQueryBuilder.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_ID, planIdList));
        }

        //分数强制排序
        String searchkeyword = recommendationPlanSearchVo.getSearchKeyword();
        if (!StringUtils.isEmpty(searchkeyword)) {
            sortOrderObjectList.add(new SortOrderObject(SortBuilders.scoreSort(), SortOrderObject.TRACK_SCORES));
        }
                ...
                if (null != brandIdMap && 0 < brandIdMap.size()) {
                    ...
                    if (!StringUtils.isEmpty(brandIdStr)) {
                        ...
                        if (null != brandIdList && 0 < brandIdList.size()) {
                            ...
                            brandBool.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_BRAND_ID_LIST, brandIdList));
    ...
    public List<RecommendationPlanVo> dispostRecommendPlanResultList(List<RecommendationPlanIndexMappingData> recommendationPlanIndexMappingDataList, Integer userId, String platformCode, Integer companyId) {
        ...
        if (null != recommendationPlanIndexMappingDataList && 0 < recommendationPlanIndexMappingDataList.size()) {
            ...
            for (RecommendationPlanIndexMappingData planIndexMappingData : recommendationPlanIndexMappingDataList) {
                ...
                recommendationPlanVo.setPlanQuotedPrice(planIndexMappingData.getPlanQuotedPrice());
                //装入对象
                ...
                if(null != planIndexMappingData.getFullHouseId() && DesignPlanType.FULLHOUSE_TABLE_TYPE == planIndexMappingData.getPlanTableType().intValue()){
                    ...
                    if ((PlatformConstant.PLATFORM_CODE_SELECT_DECORATION.equals(platformCode)) ||
                            (PlatformConstant.PLATFORM_CODE_MINI_PROGRAM.equals(platformCode) &&
                                    Objects.equals(2501,companyId))) {
                        Map<String, Object> map = null;
                        map = nodeInfoBizService.getNodeData(planIndexMappingData.getFullHouseId(),
    ...
    public SearchObjectResponse searchGoodDesign(RecommendationPlanSearchVo recommendationPlanSearchVo, PageVo pageVo) throws RecommendationPlanSearchException {
        ...
        if (null != productIdList && 0 < productIdList.size()) {
            boolQueryBuilder.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLANA_PRODUCT_ID_LIST, productIdList));
        }

        //发布状态
        Integer releaseStatus = recommendationPlanSearchVo.getReleaseStatus();
        if (null != releaseStatus) {
            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_RELEASE_STATUS, releaseStatus));
        }

        //方案类型
        List<Integer> typeList = recommendationPlanSearchVo.getRecommendationPlanTypeList();
        if (null != typeList && 0 < typeList.size()) {
            boolQueryBuilder.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_RECOMMENDATIONPLAN_TYPE, typeList));
        }
        ...
        if (null != companyId && 0 < companyId) {
            BoolQueryBuilder companyInfoBoolQueryBuilder = getBoolQueryBuildersOfCompanyIdList(recommendationPlanSearchVo);
            if (null != companyInfoBoolQueryBuilder) {
                boolQueryBuilder.must(companyInfoBoolQueryBuilder);
            }
        }
        ...
        if (recommendationPlanSearchVo.getPlatformCode() != null && PlatformConstant.PLATFORM_CODE_SELECT_DECORATION.equals(recommendationPlanSearchVo.getPlatformCode())){
            // 方案来源
            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_SOURCE, RecommendationPlanPo.DIY_PLAN_SOURCE));
        }
        ...
        groupPrimaryBool.should(new ScriptQueryBuilder(script));
        groupPrimaryBool.should(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLATFORM_GROUP_PRIMARY_ID, 0));
        boolQueryBuilder.must(groupPrimaryBool);
    ...
    public SearchObjectResponse searchRecommendationPlanByIds(List<Integer> recommendedPlanIdList, List<Integer> fullHousePlanIdList, RecommendationPlanSearchVo recommendedPlanVo, PageVo pageVo) throws RecommendationPlanSearchException {
        ...
        if (null != recommendedPlanIdList && 0 < recommendedPlanIdList.size()) {
            ...
            recommendedPlanQueryBuilder.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_ID, recommendedPlanIdList));
            //空间面积
            ...
            if (null != spaceAreas && spaceAreas > 0) {
                ...
                //方案还没选面积，适用面积为空
                spaceAreasBool.should(QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_APPLY_ALL_SPACE_AREAS_LIST)));
                recommendedPlanQueryBuilder.must(spaceAreasBool);
            }
            //空间类型
            if (null != houseType && houseType > 0) {
                recommendedPlanQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_HOUSE_TYPE, houseType));
            }
        }
        ...
        if (null != fullHousePlanIdList && 0 < fullHousePlanIdList.size()) {
            ...
            fullHouseQueryBuilder.must(QueryBuilders.termsQuery(QueryConditionField.QUERY_CONDITION_FIELD_FULL_HOUSE_ID, fullHousePlanIdList));
            //空间类型
            fullHouseQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_HOUSE_TYPE, SystemDictionaryType.SYSTEM_DICTIONARY_HOUSETYPE_FULLROOM_VALUE));
        }
        ...
        if (null != recommendedPlanQueryBuilder && null != fullHouseQueryBuilder) {
            ...
            boolQueryBuilder.must(planIdQueryBuilder);
        } else if (null != recommendedPlanQueryBuilder) {
            boolQueryBuilder.must(recommendedPlanQueryBuilder);
        } else if (null != fullHouseQueryBuilder) {
            boolQueryBuilder.must(fullHouseQueryBuilder);
        } else {
        ...
        if (null != designStyleId && 0 < designStyleId) {
            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_DESIGN_STYLEID, designStyleId));
        }
        ...
        BoolQueryBuilder boolQueryBuildersOfDecoratePrice = getBoolQueryBuildersOfDecoratePrice(decoratePriceType, decoratePriceRenge);
        if (null != boolQueryBuildersOfDecoratePrice) {
            boolQueryBuilder.must(boolQueryBuildersOfDecoratePrice);
        }
...
//
//        boolQueryBuilder.must(orderingBool);

        //在随选网展示
        if (null != shopRecommendedPlanVo.getShowInSXWFlag()) {
            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_SHOW_IN_SXW_FLAG, shopRecommendedPlanVo.getShowInSXWFlag()));
        }
        ...
        if (null != houseType && 0 < houseType) {
            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_HOUSE_TYPE, houseType));
        } else if (org.apache.commons.lang3.StringUtils.isNotBlank(shopRecommendedPlanVo.getUniqueId())) {
            houseType = metaDataService.getSpaceTypeByUniqueId(shopRecommendedPlanVo.getUniqueId());
            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_HOUSE_TYPE, houseType));
        }
        ...
        if (null != spaceAreas && spaceAreas > 0) {
            ...
            //测试方案还没选面积，适用面积为空
            spaceAreasBool.should(QueryBuilders.boolQuery().mustNot(QueryBuilders.existsQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_APPLY_ALL_SPACE_AREAS_LIST)));
            boolQueryBuilder.must(spaceAreasBool);
        }
        ...
        if ((null == shopId || shopId == 0) && PlatformConstant.PLATFORM_CODE_SELECT_DECORATION.equals(shopRecommendedPlanVo.getPlatformCode())){
            // 方案来源
            // xxx : XZW-491 【后台】【随选网】推荐方案管理后台
            // create by zhoujc at  2019/3/18 10:28.
//            boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_SOURCE, RecommendationPlanPo.DIY_PLAN_SOURCE));
        }
		...
		if (shopRecommendedPlanVo.getPlanId() != null && shopRecommendedPlanVo.getPlanId() > 0) {
			nestedBool.must(QueryBuilders.termQuery(planShopInfoList + "." + QUERY_CONDITION_FIELD_RECOMMENDATION_PLANA_PLAN_ID, shopRecommendedPlanVo.getPlanId()));
		}
        if (!CollectionUtils.isEmpty(shopRecommendedPlanVo.getPlanIds())) {
			nestedBool.must(QueryBuilders.termsQuery(planShopInfoList + "." + QUERY_CONDITION_FIELD_RECOMMENDATION_PLANA_PLAN_ID, shopRecommendedPlanVo.getPlanIds()));
        }
        if (!CollectionUtils.isEmpty(shopRecommendedPlanVo.getExcludePlanIds())) {
            nestedBool.mustNot(QueryBuilders.termsQuery(planShopInfoList + "." + QUERY_CONDITION_FIELD_RECOMMENDATION_PLANA_PLAN_ID, shopRecommendedPlanVo.getExcludePlanIds()));
        }
        ...
        //方案装修报价查询条件
        //随选网全屋报价区间过滤
        if (null != shopRecommendedPlanVo.getPlanQuotedPriceLowest() && null != shopRecommendedPlanVo.getPlanQuotedPriceHighest()) {
            if (shopRecommendedPlanVo.getPlanQuotedPriceLowest() == 100 && shopRecommendedPlanVo.getPlanQuotedPriceHighest() == 2000) {
                log.debug("此时不做过滤...");
            } else {
                BoolQueryBuilder planQuotePriceQuery = QueryBuilders.boolQuery();
                planQuotePriceQuery.must(QueryBuilders.rangeQuery(QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_QUOTED_PRICE_LIST + "." + QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_QUOTED_PRICE_LIST_PRICE)
                        .gte(shopRecommendedPlanVo.getPlanQuotedPriceLowest())
                        .lte(shopRecommendedPlanVo.getPlanQuotedPriceHighest())
                );
                NestedQueryBuilder nestedQuotedPriceQuery = QueryBuilders.nestedQuery(QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_QUOTED_PRICE_LIST,
                        planQuotePriceQuery,
                        ScoreMode.Max
                );
                boolQueryBuilder.must(nestedQuotedPriceQuery);
            }
        }
        ...
        sortOrderObjectList.add(
                new SortOrderObject(
                        QueryConditionField.QUERY_CONDITION_FIELD_SORT,
                        SortOrder.DESC,
                        SortMode.MAX,
                        SortOrderObject.DEFAULT_SORT
                )
        ...
        sortOrderObjectList.add(
                new SortOrderObject(
                        QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_MODIFY_TIME,
                        SortOrder.DESC,
                        SortMode.MAX,
                        SortOrderObject.DEFAULT_SORT
                )
...
//        );
    ...
    public SearchObjectResponse getTopDesignPlanRecommendInfo(RecommendationPlanSearchVo recommendationPlanVo) {
        ...
        boolQueryBuilder.must(QueryBuilders.rangeQuery(QueryConditionField.QUERY_CONDITION_FIELD_ORDERING).gt(Integer.valueOf(0)));

//        if(recommendationPlanVo != null ) {
//            Integer spaceType = recommendationPlanVo.getHouseType();
//            if(spaceType != null) {
//                boolQueryBuilder.must(QueryBuilders.termQuery(QueryConditionField.QUERY_CONDITION_FIELD_RECOMMENDATION_PLAN_HOUSE_TYPE, spaceType));
//            }
//        }
        matchQueryList.add(boolQueryBuilder);
        List<SortOrderObject> sortOrderObjectList = new ArrayList<>();
    ...
    @Override
    public List<RecommendationPlanVo> dispostRecommendPlanResultList(List<RecommendationPlanIndexMappingData> recommendationPlanIndexMappingDataList, Integer userId, String platformCode, Integer companyId, String cityCode) {
        // xzw-520 处理方案区域报价
        if (org.apache.commons.lang3.StringUtils.isNotBlank(cityCode)) {
            for (RecommendationPlanIndexMappingData data : recommendationPlanIndexMappingDataList) {
                if(data.getPlanQuotedPriceList() == null ){
                    continue;
                }
                data.getPlanQuotedPriceList().stream().filter(it -> it.getCityCode().equals(cityCode)).findFirst().ifPresent(it -> data.setPlanQuotedPrice(it.getPrice()));
            }
        }
        return this.dispostRecommendPlanResultList(recommendationPlanIndexMappingDataList, userId, platformCode, companyId);
    }

    @Override
    public void setFullPlanRenderState(Integer houseId, String authorization, List<RecommendationPlanVo> recommendationPlanVoList) {
        if (houseId == null || StringUtils.isEmpty(authorization)) {
            return;
        }
        String planIds = recommendationPlanVoList.stream().map(RecommendationPlanVo::getId).map(Objects::toString).collect(Collectors.joining(","));
        String url = String.format(renderHost + "/app/web/design/preRenderResult/getList.htm?houseId=%s&hasPrice=true&fullHouseRecommendPlanIds=%s", houseId, planIds);

        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_JSON);
        headers.set("Authorization", authorization);
        HttpEntity<String> requestEntity = new HttpEntity<>(null, headers);
        ResponseEntity<ResponseEnvelope> result = restTemplate.exchange(url, HttpMethod.GET, requestEntity, ResponseEnvelope.class);
        ResponseEnvelope body = result.getBody();
        log.info("preRenderResult/getList body: {}", body);
        if (body != null && body.isSuccess()) {
            List datalist = body.getDatalist();
            for (Object dto : datalist) {
                Map<String, String> it = dto instanceof Map ? ((Map) dto) : null;
                if (it == null) {
                    continue;
                }
                Object id = it.get("fullHouseRecommendPlanId");
                recommendationPlanVoList.stream()
                        .filter(t -> t.getId().toString().equals(id + ""))
                        .findFirst()
                        .ifPresent(t -> t.setRenderStateInfo(it));
            }
        }
    }
