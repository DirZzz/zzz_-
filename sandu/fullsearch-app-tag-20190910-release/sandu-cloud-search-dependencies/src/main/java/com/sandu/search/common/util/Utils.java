...
public class Utils {
     ...
     * @return
     */
    public static String getMonthBefore(Date date, int month, int type) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM");
        Calendar now = Calendar.getInstance();
        now.setTime(date);
        now.set(Calendar.MONTH, now.get(Calendar.MONTH) - month);
        if (0 == type) {
            return format.format(now.getTime()) + "-01 00:00:00";
        } else {
            return format.format(now.getTime()) + "-31 23:59:59";
        }
    }
}