...
public class Utils {
	...
	public static List<LocalDate> getWeekOrMonthDateRangeList(String nowDate, String type){
		List<LocalDate> resultList = new ArrayList<>();
		if (StringUtils.isEmpty(nowDate) || StringUtils.isEmpty(type)){
			return resultList;
		}
		LocalDate localDate = LocalDate.parse(nowDate);
		if(type.equals("1")){
			LocalDate yesterday = LocalDate.parse(nowDate).minusDays(1);
