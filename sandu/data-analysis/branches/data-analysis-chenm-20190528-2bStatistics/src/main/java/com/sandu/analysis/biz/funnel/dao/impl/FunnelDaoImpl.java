...
public class FunnelDaoImpl implements FunnelDao {
	...
	@Override
	public void insertBeforeDelete(List<BigdataFunnalAnalysisResultDO> list) {
		if(list == null || list.size() == 0) {
			System.out.println("warn, function = FunnelDao.insertBeforeDelete, message = (list == null || list.size() == 0) = true");
			return;
		}
		
		Set<BigdataFunnalAnalysisResultQuery> bigdataFunnalAnalyzeResultQuerySet = new HashSet<BigdataFunnalAnalysisResultQuery>();
		for(BigdataFunnalAnalysisResultDO item : list) {
	...
	private void delete(Set<BigdataFunnalAnalysisResultQuery> querySet) {
	...
	private void delete(BigdataFunnalAnalysisResultQuery query) {
	...
	private void delete(List<Long> idList) {
		...
		try {
			...
			String sql = "update bigdata_funnal_analysis_result set is_deleted = 1 where id = ?";
	...
	@Override
	public List<Long> select(BigdataFunnalAnalysisResultQuery query) {
		if(query == null || query.getEndTime() == null || query.getFunnelId() == null || query.getStartTime() == null) {
		...
		try {
			QueryRunner qr = new QueryRunner(JdbcUtils.getDataSource());
			/*String sql = "select id from bigdata_funnal_analysis_result "
			...
			String sql = "select id from bigdata_funnal_analysis_result where funnel_id = ? and start_time = ? and end_time = ?";
	...
	private void insert(List<BigdataFunnalAnalysisResultDO> list) {
		if(list == null || list.size() == 0) {
		...
		try {
			QueryRunner qr = new QueryRunner(JdbcUtils.getDataSource());
			String sql = "insert into bigdata_funnal_analysis_result (start_time, end_time, " + 
					"funnel_id, node_id, node_name, " + 
					"node_uv, node_pv, node_seq, " + 
					"creator, gmt_create, modifier, " + 
					"gmt_modified, is_deleted, remark, channel" + 
					") values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
			Object params[][] = new Object[list.size()][];
			for (int i = 0; i < list.size(); i++) {
				BigdataFunnalAnalysisResultDO bigdataFunnalAnalyzeResultDO = list.get(i);
				params[i] = new Object[] {
						...
						bigdataFunnalAnalyzeResultDO.getChannel()
						};
			}
			qr.batch(sql, params);
		} catch (SQLException e) {
