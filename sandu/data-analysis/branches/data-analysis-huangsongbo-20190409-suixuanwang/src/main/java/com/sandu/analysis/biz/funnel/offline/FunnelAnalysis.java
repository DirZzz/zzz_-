...
public class FunnelAnalysis {
	...
	public static void main(String[] args) {
		...
		for(Long key : bigdataFunnalDetailBOListMap.keySet()) {
			...
			Map<String, List<Integer>> nodeUvMap = javaPairRDD.foldByKey(initList, (list1, list2) -> {
				List<Integer> resultList = new ArrayList<>();
				for (int index = 0; index < list1.size(); index++) {
			...
			insertToMysql(localDate, bigdataFunnalDetailBOListItem, nodeNumMap, nodeUvMap);
		}
	 ...
	 * @param bigdataFunnalDetailBOListItem 漏斗模型
	 * @param nodeNumMap 漏斗节点次数map(key = channel)
	 * @param nodeUvMap 漏斗节点uv map(key = channel)
	 */
	private static void insertToMysql(LocalDate localDate, List<BigdataFunnalDetailBO> bigdataFunnalDetailBOList,
		...
		for(String key : nodeNumMap.keySet()) {
			...
			insertToMysql(localDate, bigdataFunnalDetailBOList, funnelNodePV, funnelNodeUV, key);
		}
	}
	...
	private static void insertToMysql(LocalDate localDate, List<BigdataFunnalDetailBO> bigdataFunnalDetailBOList, List<Integer> funnelNodePV, List<Integer> funnelNodeUV, String channel) {
		if(bigdataFunnalDetailBOList == null || bigdataFunnalDetailBOList.size() == 0) {
		...
		List<BigdataFunnalAnalysisResultDO> list = new ArrayList<BigdataFunnalAnalysisResultDO>();
		/*for(BigdataFunnalDetailBO bigdataFunnalDetailBO : bigdataFunnalDetailBOList) {*/
		for (int index = 0; index < bigdataFunnalDetailBOList.size(); index++) {
			BigdataFunnalDetailBO bigdataFunnalDetailBO = bigdataFunnalDetailBOList.get(index);
			
			BigdataFunnalAnalysisResultDO bigdataFunnalAnalyzeResultDO = new BigdataFunnalAnalysisResultDO();
			...
			bigdataFunnalAnalyzeResultDO.setChannel(channel);
