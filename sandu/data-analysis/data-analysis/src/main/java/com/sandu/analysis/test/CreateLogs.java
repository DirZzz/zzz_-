...
public class CreateLogs {
	...
	public void test004() {
		// 用户uuidList
		List<Integer> userUuidList = new ArrayList<Integer>();
		for (int index = 0; index < 2000; index++) {
			userUuidList.add(index);
		}
		
		List<String> appidList = new ArrayList<String>(Arrays.asList("asdfasdfasd", "vfhfgasdf"));
		
		List<String> channelList = new ArrayList<String>(Arrays.asList("toutiao", "taoxinwen"));
		
		List<Integer> newUserList = this.getRandomList(userUuidList, 50);
		
		for(Integer integer : newUserList) {
			System.out.println(integer + "\t" + getRandomElement(appidList) + "\t" + getRandomElement(channelList));
		}
