...
public class FunnelAnalysis {
	...
	public static void main(String[] args) {
		...
		if(ConfigConstants.RUN_ENV_LOCAL.equals(AnalysisConstants.RUN_ENV)) {
			...
			if(!isExist) {
				...
				insertToMysql(localDate, bigdataFunnelDetailBOList, null, null, null, null);
		...
		for(Long key : bigdataFunnelDetailBOListMap.keySet()) {
			...
			// ==========识别新用户, add by huangsongbo 2019.07.12, 新用户标记 = 1, else = 0 ->start
			// key = uuid		isNewUser, value = sorceList(eg: 5, 3, 0)
			.mapToPair(t -> {
				String newUserFlag = null;
				if (newUserUuidList.contains(t._1)) {
					newUserFlag = "1";
				} else {
					newUserFlag = "0";
				}
				return new Tuple2<String, List<Integer>>(t._1 + AnalysisConstants.SPLIT_REGEX + newUserFlag, t._2);
			})
			// ==========识别新用户, add by huangsongbo 2019.07.12, 新用户标记 = 1, else = 0 ->end
			// key = channel		isNewUser, value = sorceList(eg: 5, 3, 0)
			.mapToPair(t -> {
				...
				return new Tuple2<String, List<Integer>>(channel + AnalysisConstants.SPLIT_REGEX + strs[1], t._2);
			})
			.cache();
			
			if(javaPairRDD.isEmpty()) {
				System.out.println("warn, function = main, message = (javaPairRDD.isEmpty()) = true");
				insertToMysql(localDate, bigdataFunnelDetailBOListItem, null, null, null, null);
				continue;
			}
			
			// ==========漏斗节点类pv统计 ->start
			// key = channel		isNewUser, value = sorceList(eg: 5, 3, 0)
			JavaPairRDD<String, List<Integer>> nodeNumMapJavaPairRDD = javaPairRDD.reduceByKey((list1, list2) -> {
				return mergeList(list1, list2);
			}).cache();
			
			// key = channel		isNewUser, value = sorceList(eg: 5, 3, 0)
			Map<String, List<Integer>> nodeNumMap = nodeNumMapJavaPairRDD
			.mapToPair(t -> {
				String[] strs = t._1.split(AnalysisConstants.SPLIT_REGEX);
				return new Tuple2<String, List<Integer>>(strs[0], t._2);
			})
			...
			// ==========漏斗节点类npv(新用户pv)统计 ->start
			// 			 0				1
			// key = channel		isNewUser, value = sorceList(eg: 5, 3, 0)
			Map<String, List<Integer>> newUserNodeNumMap = nodeNumMapJavaPairRDD
					.filter(t -> {
						String[] strs = t._1.split(AnalysisConstants.SPLIT_REGEX);
						if (StringUtils.equals(strs[1], "1")) {
							return true;
						}
						return false;
					})
			...
			System.out.println("------newUserNodeNumMap = " + newUserNodeNumMap);
			// ==========漏斗节点类npv(新用户pv)统计 ->end
			
			// ==========漏斗节点uv统计 ->start
			// key = channel		isNewUser, value = sorceList(eg: 5, 3, 0)
			JavaPairRDD<String, List<Integer>> nodeUvMapJavaPairRDD = javaPairRDD.mapValues(l -> {
			...
			Map<String, List<Integer>> nodeUvMap = nodeUvMapJavaPairRDD.mapToPair(t -> {
				String[] strs = t._1.split(AnalysisConstants.SPLIT_REGEX);
				return new Tuple2<String, List<Integer>>(strs[0], t._2);
			})
			.reduceByKeyLocally((list1, list2) -> {
			...
			// ==========漏斗节点类nuv(新用户uv)统计 ->start
			// 			 0				1
			// key = channel		isNewUser, value = sorceList(eg: 5, 3, 0)
			Map<String, List<Integer>> newUserNodeUvMap = nodeUvMapJavaPairRDD
					.filter(t -> {
						String[] strs = t._1.split(AnalysisConstants.SPLIT_REGEX);
						if (StringUtils.equals(strs[1], "1")) {
							return true;
			...
			System.out.println("------newUserNodeUvMap = " + newUserNodeUvMap);
			// ==========漏斗节点类nuv(新用户uv)统计 ->end
			
			insertToMysql(localDate, bigdataFunnelDetailBOListItem, nodeNumMap, nodeUvMap, newUserNodeNumMap, newUserNodeUvMap);
		}
	 ...
	 * @param newUserNodeUvMap 
	 * @param newUserNodeNumMap 
	 */
	private static void insertToMysql(
			LocalDate localDate, 
			List<BigdataFunnelDetailBO> bigdataFunnelDetailBOList,
			Map<String, List<Integer>> nodeNumMap, 
			Map<String, List<Integer>> nodeUvMap, 
			Map<String, List<Integer>> newUserNodeNumMap, 
			Map<String, List<Integer>> newUserNodeUvMap
			) {
		if (nodeNumMap == null) {
			System.out.println("nodeNumMap == null, return");
			return;
		}
		if(nodeUvMap == null) {
			System.out.println("nodeUvMap == null, return");
			return;
		}
		for (String key : nodeNumMap.keySet()) {
			List<Integer> funnelNodePV = nodeNumMap.get(key);
			List<Integer> funnelNodeUV = nodeUvMap.get(key);
			List<Integer> funnelNodeNPV = newUserNodeNumMap.get(key);
			List<Integer> funnelNodeNUV = newUserNodeUvMap.get(key);
			insertToMysql(localDate, bigdataFunnelDetailBOList, funnelNodePV, funnelNodeUV, funnelNodeNPV, funnelNodeNUV, key);
		}
	}
	...
	private static void insertToMysql(
			LocalDate localDate, 
			List<BigdataFunnelDetailBO> bigdataFunnelDetailBOList, 
			List<Integer> funnelNodePV, 
			List<Integer> funnelNodeUV, 
			List<Integer> funnelNodeNPV, 
			List<Integer> funnelNodeNUV, 
			String channel
			) {
		if(bigdataFunnelDetailBOList == null || bigdataFunnelDetailBOList.size() == 0) {
		...
		for (int index = 0; index < bigdataFunnelDetailBOList.size(); index++) {
			...
			/*bigdataFunnelAnalyzeResultDO.setNodeNpv(funnelNodeNPV.get(index));*/
			
			int indexTemp = index;
			
			bigdataFunnelAnalyzeResultDO.setNodeNpv(Optional.ofNullable(funnelNodeNPV).map(i -> i.get(indexTemp)).map(i -> i).orElse(0));
			
			/*bigdataFunnelAnalyzeResultDO.setNodeNuv(funnelNodeNUV.get(index));*/
			bigdataFunnelAnalyzeResultDO.setNodeNuv(Optional.ofNullable(funnelNodeNUV).map(i -> i.get(indexTemp)).map(i -> i).orElse(0));
