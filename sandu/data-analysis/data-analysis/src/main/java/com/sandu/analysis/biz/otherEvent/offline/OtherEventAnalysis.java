...
public class OtherEventAnalysis {
	...
	public static void main(String[] args) {
		...
		.filter(str -> {
			...
			String event = eventMap.get(EVENTPROPERTY_ENUM.event.getName());
			if (StringUtils.isEmpty(event)) {
				event = eventMap.get(EVENTPROPERTY_ENUM.eventcode.getName());
			}
			String code = strs[3] + AnalysisConstants.SPLIT_REGEX + event + AnalysisConstants.SPLIT_REGEX + strs[5];
	...
	private static void InsertIntoDB(Map<String, Long> pvResultMap, Map<String, Long> uvResultMap,
		...
		if(pvUvResult == null || pvUvResult.size() == 0) {
			System.out.println("warn, function = HouseEventAnalysis.InsertIntoDB, message = (pvUvResult == null || pvUvResult.size() == 0) = true");
			return;
		}
