...
public class UserRetentionResultDaoImpl implements UserRetentionResultDao {
	...
	public void insertBeforeDelete(UserRetentionResultDO userRetentionResultDO) {
		...
		userRetentionResultQuery.setAppId(userRetentionResultDO.getAppId());
	...
	private void delete(UserRetentionResultQuery query) {
		...
		if(query == null || query.getEndTime() == null || query.getStartTime() == null || query.getAppId() == null) {
			...
			System.out.println("info, function = " + functionName + ", message = query.getAppId() = " + query.getAppId());
			return;
		}
	...
	private List<Long> select(UserRetentionResultQuery query) {
		...
		List<Long> list = null;
		String sql = "select id from bigdata_user_retention_result where start_time = ? and end_time = ? and app_id = ?";
		Object[] params = new Object[] {simpleDateFormat.format(query.getStartTime()), simpleDateFormat.format(query.getEndTime()), query.getAppId()};
