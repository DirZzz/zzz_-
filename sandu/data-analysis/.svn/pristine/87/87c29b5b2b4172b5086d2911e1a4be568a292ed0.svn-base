...
public class HouseStatisticsDaoImpl implements HouseStatisticsDao {
    ...
    public List<HouseInfoDto> selectHouseArea(List<String> houseIdList) {
        ...
        String sql = "select a.id as houseId,c3.area_code as provinceCode, c3.area_name as provinceName,c2.area_code as cityCode, c2.area_name as cityName, a.house_common_code as houseName, a.house_code houseCode,b.living_name as livingName "
                + "from app_online_30.base_house a "
                + "LEFT JOIN app_online_30.base_living b ON b.id = a.living_id and b.is_deleted = 0 "
                + "LEFT JOIN app_online_30.base_area c1 on c1.area_code = b.area_id "
                + "LEFT JOIN app_online_30.base_area c2 ON c2.area_code = c1.pid "
                + "LEFT JOIN app_online_30.base_area c3 ON c3.area_code = c2.pid "
    ...
    public List<HouseInfoDto> selectNewHouseCount(String startTime, String endTime, String type) {
        ...
        if (Objects.equals(type, HouseEventConstants.HOUSE_DAY_TYPE)) {
            sql = "select c3.area_code as provinceCode, c3.area_name as provinceName,c2.area_code as cityCode, c2.area_name as cityName, count(a.id) as newHouseCount "
                    + "from app_online_30.base_house a "
                    + "LEFT JOIN app_online_30.base_living b ON b.id = a.living_id and b.is_deleted = 0 "
                    + "LEFT JOIN app_online_30.base_area c1 on c1.area_code = b.area_id "
                    + "LEFT JOIN app_online_30.base_area c2 ON c2.area_code = c1.pid "
                    ...
                    + "group by c3.area_code,c2.area_code";
        } else if (Objects.equals(type, HouseEventConstants.HOUSE_HOUR_TYPE)) {
            sql = "select DATE_FORMAT(a.gmt_create,'%Y-%m-%d %h') as perHour,a.gmt_create as createTime, c3.area_code as provinceCode, c3.area_name as provinceName,c2.area_code as cityCode, c2.area_name as cityName, count(a.id) as newHouseCount "
                    + "from app_online_30.base_house a "
                    + "LEFT JOIN app_online_30.base_living b ON b.id = a.living_id and b.is_deleted = 0 "
                    + "LEFT JOIN app_online_30.base_area c1 on c1.area_code = b.area_id "
                    + "LEFT JOIN app_online_30.base_area c2 ON c2.area_code = c1.pid "
                    + "LEFT JOIN app_online_30.base_area c3 ON c3.area_code = c2.pid "
                    + "where a.gmt_create BETWEEN '" + startTime + "' AND '" + endTime + "'"
                    + "group by DATE_FORMAT(a.gmt_create,'%Y-%m-%d %h'),c3.area_code,c2.area_code";
        }
