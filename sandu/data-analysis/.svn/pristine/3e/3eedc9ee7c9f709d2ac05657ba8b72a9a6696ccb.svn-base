 ...
 *
 */
public class FunnelAnnlysis {

	public static void main(String[] args) {
		
		// 获取时间: 分析那一天的log
		// 返回eg: 201904/20/
		LocalDate localDate = getLocalDate(args);
		...
		if(ConfigConstants.RUN_ENV_LOCAL.equals(AnalysisConstants.RUN_ENV)) {
			...
			hdfsFileDir = AnalysisConstants.HDFS_DIR + dataDirInfo;
			// 检测hdfs中有没有这个目录, 如果没有则创建目录
			boolean isExist = getIsExist(hdfsFileDir);
			if(!isExist) {
		...
		JavaRDD<String> fileRDD = javaSparkContext.textFile(hdfsFileDir).filter(str -> {
		...
		for(Long key : bigdataFunnalDetailBOListMap.keySet()) {
			...
			.map(str -> {
				...
				return strs[0] + AnalysisConstants.SPLIT_REGEX + (funnelPageList.indexOf(event)) + AnalysisConstants.SPLIT_REGEX + time;
			})
			// 过滤和漏斗无关的事件
			.filter(str -> {
				String index = str.split(AnalysisConstants.SPLIT_REGEX)[2];
				return !"-1".equals(index);
			})
			// 转化为
			...
			.mapValues(intIterable -> {
				...
				while (intIterator.hasNext()) {
					Integer intItem = intIterator.next();
				...
				/*List<Integer> list = Lists.newArrayList(intIterable);*/
	...
	/**
	 * 
	 * 检测hdfs中有没有该目录, 如果没有则创建该目录
	 * 
	 * @author huangsongbo
	 * @param hdfsFileDir
	 */
	private static boolean getIsExist(String hdfsFileDir) {
	...
	private static LocalDate getLocalDate(String[] args) {
