...
//            refundParam.setOperator(loginUser.getId().longValue());
            ...
            logger.info("退款接口完成 =>{}" + obj);
            return obj;
        } catch (BizException e) {
            logger.error("业务异常 =>{}", e);
            return new ResponseEnvelope(true, e.getMessage(), false,e.getCode() + "", false);
        } catch (Exception e) {
            logger.error("未知错误:", e);
            return new ResponseEnvelope(true, "未知错误", false,"500", false);
        }
    }
    ...
    private void validateSign(RefundParam refundParam) {
        ...
        if(!sign.equals(refundParam.getSign())){
            throw new BizException(212313,"签名错误");
        }
    }
    ...
    public static void main(String[] args) {
        ...
        String sign = MD5SignTool.MD5Encode("operator="+60531
                + "&platformCode="+"pc2b"
                + "&timestamp="+timestamp
                + "&key="+ENCRYPT_KEY
                +"&refundFee="+1
