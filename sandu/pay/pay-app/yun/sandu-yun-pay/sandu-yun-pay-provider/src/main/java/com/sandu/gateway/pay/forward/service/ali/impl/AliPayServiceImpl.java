...
public abstract class AliPayServiceImpl extends  BasePayServiceImpl{
	...
	protected String updateAndPackageRefundResult(PayTradeRefundRecord tradeRefundRecord,String jsonResult) {
		...
		if ("N".equals(responseMap.get("fund_change"))){
            throw new com.sandu.pay.common.exception.BizException(com.sandu.pay.common.exception.ExceptionCodes.CODE_10010401,"支付宝退款异常:" +"订单已全额退款" );
        }
		logger.info("支付宝退款返回结果 =>{},",responseMap);
...
//		packageRetParams.put("refund_fee",responseMap.get("refund_fee"));
		...
		logger.info("支付宝退款返回参数 =>{}",resultParams);
